...
\begin{align}
	\frac{\partial}{\partial \lambda} \int_{\Omega_{s }} \frac{\mathrm{d}^{2} \bm{k}}{\left(2 \pi\right)^2} \mathcal{F} \left(\bm{k }\right) = \int_{\Omega_{s }} \frac{\mathrm{d}^{2} \bm{k}}{\left(2 \pi\right)^2} \partial \lambda \mathcal{F}  \left(\bm{k }\right) + \oint_{\partial \Omega_{s }} \frac{\mathrm{d} \ell}{\left(2 \pi\right)^{2}} v^{\perp}_{\lambda} \mathcal{F} \left(\bm{k}\right).
\end{align}
...
我们首先处理第一项，第一项的积分是
\begin{align}
	I_{1,\alpha} = \sum_{s} \int_{\Omega_{s }} \frac{\mathrm{d}^{2} \bm{k}}{\left(2 \pi\right)^2}  \partial_{k_{\alpha}} \mathrm{Re} \mathrm{Tr} \left[\mathcal{P} \mathcal{P}_{\lambda} \mathcal{Q} \mathcal{Z} \right] 
\end{align}
二维高斯定理给出
\begin{align}
...
由此可以将第一项的求和积分写成
\begin{align}
	I_{1,\alpha} = \sum_{s } \oint_{\partial \Omega_{s }} \frac{\mathrm{d} \ell }{\left(2 \pi\right)^2} n_{s,\alpha} \mathcal{A}_{s}.
\end{align}
现在，假设某条费米面是由\(\mu\)穿过\(E_{m} \left(\bm{k}\right)\)所得到的，费米面的两侧分别是\(E_{l} < \mu\)和\(E_{l} > \mu\), 定义法向量
\begin{align}
...
\begin{align}
	\int_{\mathrm{mBZ}} \mathrm{d}^{2} \bm{k} \delta \left( E_{l} - \mu\right) \partial_{k_{\alpha}} E_{l}  \Delta_{l} \mathcal{A} = \oint_{E_{l} = \mu} \frac{\mathrm{d} \ell}{\left|\nabla_{\bm{k }} E_{l}\right|} \partial_{k_{\alpha}} E_{l}  \Delta_{l} \mathcal{A}.
\end{align}
就可以得到
\begin{align}
	I_{1,\alpha} = \int_{\mathrm{mBZ}} \frac{\mathrm{d}^{2} \bm{k}}{\left(2 \pi\right)^{2}} \sum_{l} \delta \left( E_{l} - \mu\right)  \partial_{k_{\alpha}} E_{l}  \Delta_{l} \left\{\mathrm{Re} \mathrm{Tr} \left[\mathcal{P} \mathcal{P}_{\lambda} \mathcal{Q} \mathcal{Z} \right] \right\}.  
\end{align}
现在我们来处理\(\mathcal{P}_{\lambda}\)
