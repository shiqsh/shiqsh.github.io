...
\subsection{异质应力下转角石墨烯的哈密顿量}
对于施加异质应力的转角石墨烯，通过对紧束缚模型中波函数的慢变场与晶格形变场做实空间中的一阶梯度展开，得到有效连续模型。对于特定的谷（我们选取 \(\bm{\sigma} \cdot \bm{p}\)），哈密顿量为
\begin{align}
	H = \begin{pmatrix}
			H_{t} & T \\  T^{\dagger} & H_{b}
		\end{pmatrix}.
\end{align}

其中的层间耦合项(intralayer coupling)具体为
\begin{align}
	\begin{split}
		H_{t, \left(n_1 n_2\right)} &= v_F \bm{p}^\pr_{t} \cdot \bm{\sigma} + \alpha \vph I_{\mr{2 \times 2}} + v_F \gamma \bm{A} \cdot \bm{\sigma}, \\
		H_{b, \left(n_1 n_2\right)} &= v_F \bm{p}^\pr_{b} \cdot \bm{\sigma} - \alpha \vph I_{\mr{2 \times 2}} - v_F \gamma \bm{A} \cdot \bm{\sigma}.
	\end{split}
\end{align}
其中\(\bm{p}^{\prime}_{t/b}\)是上层和下层的动量，分别是
\begin{align}
	\begin{split}
		\bm{p}^{\prime}_{t} &= \bm{k} - \left(n_1 \bm{b}^{\prime}_{m;1} + n_2 \bm{b}^{\prime}_{m;2}\right) \\
		\bm{p}^{\prime}_{b} &= \bm{k} - \left(\bm{q}^{\prime}_{1} + n_1 \bm{b}^{\prime}_{m;1} + n_2 \bm{b}^{\prime}_{m;2}\right). 
	\end{split}
\end{align}
这里\(\bm{b}^{\prime}_{m;1} \)和\(\bm{b}^{\prime}_{m;2}\) 分别是施加异质应力后莫尔布里渊区中的倒格矢基矢，\(\bm{q}^{\prime}_{1}\)是施加异质应力后上下层的转移动量，三者的表达式分别为
\begin{align}
		\bm{b}^{\prime}_{m; 1/2}  = \p{\mathcal{S} + \mathcal{T}}^{T} \bm{b}_{1/2},
	\quad
	\bm{q}^{\prime}_{1} = \p{\mathcal{S} + \mathcal{T}}^{T} \bm{K}.
\end{align}
\(\bm{b}_{1}\)和\(\bm{b}_{2}\)分别是单层石墨烯中倒格矢基矢，\(\bm{K}\)是狄拉克点坐标，具体为
\begin{align}
...
\end{align}
\(a = 2.46\)\AA 为单层石墨烯的晶格常数。上式中的\(\mathcal{S}\)和\(\mathcal{T}\)分别是应力张量与转动张量，在小角近似下可以写成
\begin{align}
	...
	\end{pmatrix},
\end{align}
其中\(R \left(\phi\right)\)是旋转矩阵，\(\phi\)是应力主轴与\(x\)轴的夹角。\(\epsilon_{1}\)和\(\epsilon_{2}\)分别是应力主轴与其垂直方向上的应变强度。通常取\(\epsilon_1 = \epsilon,\, \epsilon_2 = - \nu \epsilon\), \(\nu = 0.16\)是石墨烯的泊松比。
\begin{align}
	\mathcal{T} = \begin{pmatrix}
		0 & - \theta \\ \theta & 0
	\end{pmatrix}.
\end{align}
\(\varphi\)是赝标量场，\(\bm{A}\)是赝矢量场，两者的定义为
\begin{align}
	\varphi & = \mathrm{Tr}\left(\mathcal{S}\right) = \epsilon_1  + \epsilon_2 ,\\
	\bm{A} & = \left(\mathcal{S}_{xx} - \mathcal{S}_{yy}, \mathcal{S}_{xy} + \mathcal{S}_{yx}\right) = \left(\left(\epsilon_1 - \epsilon_2\right) \cos{2\phi}, \left(\epsilon_1 - \epsilon_2\right) \sin{2 \phi} \right).
\end{align}
这里我们忽略了转角对于层内耦合项中泡利矩阵的影响，也就是\(\bm{\sigma}^{\pm te} \rightarrow \\bm{\sigma}\)。在小角近似下，这样的处理是合理的。同时忽略掉转角对于泡利矩阵的影响，也使得我们所考虑的连续模型中具有粒子空穴对称性。

对于层间跃迁项(interlayer tunneling)，本文不保留完整梯度展开中的非局域层间隧穿修正，而采用最小 BM 近似：仅保留第一壳层中的三个转移动量，并忽略梯度耦合项以及广义层间隧穿中的 \(\sigma_3\) 分量。于是层间项写为标准 BM 形式。

在实空间中，BM 层间隧穿项可以写为
\begin{align}
	T \left(\bm{r}\right) = \sum_{i = 1}^{3} e^{i \bm{q}^{\prime}_{j} \cdot \bm{r}} \left\{ w_0 I_{2 \times 2} + w_1 \left[\sigma_{1} \cos{\frac{2 \pi}{3} \left(j - 1\right)} + \sigma_{2} \sin{\frac{2 \pi}{3} \left(j - 1\right)} \right]  \right\} 
\end{align}
由于本文实际采用的是平面波基矢，需要将上式做傅里叶变换到动量空间。定义倒格矢
\begin{align}
	\bm G_{\left(n_1 n_2\right)}^\prime = n_1\bm b_{m;1}^\prime+n_2\bm b_{m;2}^\prime,
\end{align}
则层间项的矩阵元为
\begin{align}
	\begin{split}
		T_{\left(n_1 n_2\right),\left(m_1 m_2\right)} &= \braket{t, \left(n_1 n_2\right) | b, \left(m_1 m_2\right) } \\ 
		& = \sum_{j=1}^{3} T_j \delta_{\bm G_{\left(n_1 n_2\right)}^\prime,\bm G_{\left(m_1 m_2\right)}^\prime+\bm q_1^\prime-\bm q_j^\prime} \\
		& = T_1 \delta_{n_1, m_1} \delta_{n_2,m_2} + T_2 \delta_{n_1, m_1} \delta_{n_2 , m_2 - 1} + T_3 \delta_{n_1, m_1 + 1} \delta_{n_2 , m_2}.
	\end{split}
\end{align}
其中三个转移动量之间的几何关系为 \(\bm{q}^{pr}_{2} =\bm{q}^{pr}_{1} + \bm{b}^{\prime}_{m;2}, \, \bm{q}^{pr}_{3} =\bm{q}^{pr}_{1} - \bm{b}^{\prime}_{m;1}\)。上述参数具体设置见下表
\begin{table*}[h!]
	\centering
