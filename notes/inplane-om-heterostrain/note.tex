...
\end{align}
现在再次考虑\(\Omega_{\pm}\)两个区域，定义\(O\)是全占据的能带集合，\(E\)是全空的能带集合。这意味着，对于区域\(\Omega_{-}\)中的\(\bm{k}\)点，其对应占据的能带集合是\(O \cup \left\{m\right\}\)，空能带集合是\(E\)；对于区域对于区域\(\Omega_{+}\)中的\(\bm{k}\)点，其对应占据的能带集合是\(O\)，空能带集合是\(E \cup \left\{m\right\}\)。由此，我们可以写出两个区域内的\(\mathcal{A}_{\pm}\). 首先, 在\(\Omega_{-}\)
\begin{align}
	\begin{split}
