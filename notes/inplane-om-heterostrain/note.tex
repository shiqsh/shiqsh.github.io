...
\begin{align}
		\boxed{
		M_{\alpha} = -\frac{e }{\hbar c } \epsilon_{\alpha \beta} \int_{\mathrm{mBZ }} \frac{\mathrm{d}^{2} \bm{k}}{\left(2 \pi\right)^2} \mathrm{Re} \, \mathrm{Tr} \left[\mathcal{P} \left(\partial_{k_{\beta}} \mathcal{P} \right) \mathcal{Q} \mathcal{Z}\right].
	}
\end{align}
...
我们首先处理第一项，第一项的积分是
\begin{align}
	I_{\alpha,1} = \sum_{s} \int_{\Omega_{s }} \frac{\mathrm{d}^{2} \bm{k}}{\left(2 \pi\right)^2}  \partial_{k_{\alpha}} \mathrm{Re} \mathrm{Tr} \left[\mathcal{P} \mathcal{P}_{\lambda} \mathcal{Q} \mathcal{Z} \right] 
\end{align}
二维高斯定理给出
\begin{align}
...
\begin{align}
	\mathcal{A} = \mathrm{Re} \mathrm{Tr} \left[\mathcal{P} \mathcal{P}_{\lambda} \mathcal{Q} \mathcal{Z} \right] .
\end{align}
由此可以将第一项的求和积分写成
\begin{align}
	I_{\alpha,1} = \sum_{s } \oint_{\partial \Omega_{s }} \frac{\mathrm{d} \ell }{\left(2 \pi\right)^2} n_{s,\alpha} \mathcal{A}_{s}.
\end{align}
...
就可以得到
\begin{align}
	I_{\alpha,1} = \int_{\mathrm{mBZ}} \frac{\mathrm{d}^{2} \bm{k}}{\left(2 \pi\right)^{2}} \sum_{l} \delta \left( E_{l} - \mu\right) \left( \partial_{k_{\alpha}} E_{l} \right) \Delta_{l} \left\{\mathrm{Re} \mathrm{Tr} \left[\mathcal{P} \mathcal{P}_{\lambda} \mathcal{Q} \mathcal{Z} \right] \right\}.  
\end{align}
...
所以
\begin{align}
	\mathcal{A} = \mathrm{Re} \sum_{\substack{n \in \mathrm{occ} \\ m \in \mathrm{emp}}} \frac{\mathrm{Tr} \left[\Pi_{n } \hat{H}_{\lambda} \Pi_{m } \mathcal{Z}\right]}{E_{n} - E_{m}} = \mathrm{Re} \sum_{\substack{n \in \mathrm{occ} \\ m \in \mathrm{emp}}} \frac{H_{\lambda,nm} \mathcal{Z}_{mn} }{E_{n } - E_{m }}.
\end{align}
...
进一步将\(\mathcal{A}\)写成
\begin{align}
	\mathcal{A} = \mathrm{Re} \sum_{\substack{n \in \mathrm{occ} \\ m \in \mathrm{emp}}} \frac{E_{n} + E_{m } - 2 \mu}{E_{n } - E_{m }} H_{\lambda, nm} z_{mn}.
\end{align}
现在再次考虑\(\Omega_{\pm}\)两个区域，定义\(O\)是完全占据的能带集合，\(E\)是完全空的能带集合。这意味着，对于区域\(\Omega_{-}\)中的\(\bm{k}\)点，其对应占据的能带集合是\(O \cup \left\{m\right\}\)，空能带集合是\(E\)；对于区域对于区域\(\Omega_{+}\)中的\(\bm{k}\)点，其对应占据的能带集合是\(O\)，空能带集合是\(E \cup \left\{m\right\}\)。由此，我们可以写出两个区域内的\(\mathcal{A}_{\pm}\). 首先, 在\(\Omega_{-}\)
\begin{align}
	\begin{split}
		\mathcal{A}_{-}  = \mathrm{Re} \left[\sum_{n \in O} \sum_{m \in E} + \sum_{n =l} \sum_{m \in E}\right] = \mathcal{A}_{0} + \mathrm{Re} \sum_{m \in E} \frac{E_{l} + E_{m } - 2 \mu}{E_{l} - E_{m }} H_{\lambda,lm} z_{ml} ,
	\end{split}
\end{align}
其中
\begin{align}
	\mathcal{A}_{0} = \mathrm{Re} \sum_{n \in O} \sum_{m \in E} \frac{E_{n} + E_{m } - 2 \mu}{E_{n } - E_{m }} H_{\lambda, nm} z_{mn}.
\end{align}
那么在\(\Omega_{+}\)中，则有
\begin{align}
	\begin{split}
		\mathcal{A}_{+} =  \mathrm{Re} \left[\sum_{n \in O} \sum_{m \in E} + \sum_{n \in O} \sum_{m = l}\right] = \mathcal{A}_{0} + \mathrm{Re} \sum_{n \in O } \frac{E_{n} + E_{l} - 2 \mu}{E_{n } - E_{l}} H_{\lambda,nl} z_{ln}.
	\end{split}
\end{align}
...
所以
\begin{align}
	\mathcal{A}_{-} = \mathcal{A}_{0} - \mathrm{Re}  \sum_{m \in E } H^{\lambda}_{lm } z_{m l}.
\end{align}
同理对于\(\mathcal{A}_{- }\),
\begin{align}
...
所以
\begin{align}
	\mathcal{A}_{+} = \mathcal{A}_{0} + \mathrm{Re} \sum_{n \in O } H_{\lambda,nl} z_{ln}.
\end{align}
两者的差为
\begin{align}
	\mathcal{A}_{-} - \mathcal{A}_{+} = - \mathrm{Re} \sum_{m \in E } H_{\lambda,lm} z_{m l} - \mathrm{Re} \sum_{n \in O } H_{\lambda,nl} z_{ln},
\end{align}
由于\(\hat{H}_{\lambda}\)和\( \hat{z} \)都是厄米的，所以有 \(H_{\lambda,ml} = \left(H_{\lambda,lm}\right)^{*}\)，同时\(z_{lm} = z^{*}_{lm }\), 所以
\begin{align}
	\mathrm{Re} H_{\lambda,lm} z_{ml} = \mathrm{Re} \left( H_{\lambda,ml} z_{lm} \right)^{*} = \mathrm{Re} H^{\lambda}_{ml} z_{lm},
\end{align}
可以将上式改写成
\begin{align}
	\mathcal{A}_{-} - \mathcal{A}_{+} = - \mathrm{Re} \sum_{m \in E}  H_{\lambda,lm} z_{ml} - \mathrm{Re} \sum_{n \in O } H_{\lambda, ln} z_{nl}  = - \mathrm{Re} \sum_{r \neq l} H_{\lambda, lr} z_{rl}
\end{align}
这里我们使用了一个虚拟变量\(r\)，为了符合我们的使用习惯，我们做指标替换\(r \rightarrow n\)和\(l \rightarrow m\)。因此，积分为
\begin{align}
	I_{\alpha, 1} = - \int_{\mathrm{mBZ}} \frac{\mathrm{d}^{2} \bm{k}}{\left(2 \pi\right)^2} \sum_{m} \delta \left(E_{m} - \mu\right) \left(\partial_{k_{\alpha}} E_{m }\right) \sum_{n \neq m } \mathrm{Re} \left[H_{\lambda,mn} z_{nm}\right]，
\end{align}
这里\(\partial_{k_{\alpha}} E_{m } = \braket{u_{m } | \hat{H}_{\alpha} | u_{m}}\)。费米面对极化率的第一项贡献为
\begin{align}
	\boxed{
		\chi^{\mathrm{FS}}_{\alpha,1} = \frac{e}{\hbar c} \epsilon_{\alpha \beta} \int_{\mathrm{mBZ}} \frac{\mathrm{d}^{2} \bm{k}}{\left(2 \pi\right)^2} \sum_{m} \sum_{n \neq m} \delta \left(E_{m } - \mu\right) \braket{u_{m} | \hat{H}_{\beta} | u_{m}} \mathrm{Re} \left[ \braket{u_{m} | \hat{H}_{\lambda} | u_{n }} \braket{u_{n} | \hat{z} | u_{m }}\right].
	}
\end{align}
这里\(m\)是与化学势相交的能带指标。
...
\begin{align}
	\chi^{\mathrm{FS }}_{\alpha,2} = -\frac{e}{\hbar c} \epsilon_{\alpha \beta} \sum_{s } \oint_{\partial \Omega_{s }} \frac{ \mathrm{d} \ell}{\left(2 \pi\right)^{2} } \left(\frac{\partial \bm{k}_{\mathrm{FS}}}{\partial \lambda} \cdot \hat{\bm{n }}_{s}\right) \mathrm{Re} \mathcal{F}_{\beta}^{s}.
\end{align}
定义费米面移动速度
\begin{align}
...
\begin{align}
	\left(\frac{\partial \bm{k}_{\mathrm{FS}}}{\partial \lambda} \cdot \hat{\bm{n}}_{-}\right) \mathrm{Re} \mathcal{F}^{-}_{\beta} + \left(\frac{\partial \bm{k}_{\mathrm{FS}}}{\partial \lambda} \cdot \hat{\bm{n}}_{+}\right) \mathrm{Re} \mathcal{F}^{+}_{\beta} = v_{l,\lambda} \left[\mathrm{Re} \mathcal{F}^{-}_{\beta} - \mathrm{Re} \mathcal{F}^{+}_{\beta}\right].
\end{align}
...
定义
\begin{align}
	\Delta_{l} \mathrm{Re} \mathcal{F}_{\beta} \equiv \mathrm{Re} \left[\mathcal{F}^{-}_{\beta} - \mathcal{F}^{+}_{\beta}\right].
\end{align}
可以写出费米面对极化率的第二项贡献为
\begin{align}
	\chi^{\mathrm{FS}}_{\alpha,2} =  \frac{e }{\hbar c} \epsilon_{\alpha \beta} \sum_{l} \oint_{E_{l} = \mu} \frac{\mathrm{d} \ell}{ \left(2 \pi\right)^2} \frac{ H_{\lambda,ll}}{\left|\nabla_{\bm{k}} E_{l }\right|} \Delta_{l } \mathrm{Re} \mathcal{F}_{\beta},
\end{align}
将线积分改写成面积分
\begin{align}
	\chi^{\mathrm{FS}}_{\alpha,2} =  \frac{e }{\hbar c} \epsilon_{\alpha \beta} \sum_{l} \int_{\mathrm{mBZ}} \frac{\mathrm{d}^{2} \bm{k}}{\left(2 \pi\right)^2} \delta \left(E_{l} - \mu\right) H_{\lambda,ll} \Delta_{l} \mathrm{Re} \mathcal{F}_{\beta}.
\end{align}
现在来处理\(\Delta_{l} \mathrm{Re} \mathcal{F}_{\beta}\), 由于
\begin{align}
	\mathrm{Re} \mathcal{F}_{\beta} = -\sum_{\substack{n \in \mathrm{occ} \\ m \in \mathrm{emp}}} \left(E_{n } + E_{m } - 2\mu\right) \mathrm{Re} \left[\braket{u_{n } | \partial_{k_{\beta}} u_{m }} \braket{u_{m } | \hat{z} | u_{n }}\right].
\end{align}
对\(\Omega_{-}\)这一侧区域有
\begin{align}
	\mathrm{Re} \mathcal{F}^{-}_{\beta} = \mathcal{F}_{\beta}^{0} - \sum_{m \in E} \left(E_{m} + E_{l} - 2\mu\right) \mathrm{Re} \left[\braket{u_{l} | \partial_{k_{\beta}} u_{m }} \braket{u_{m } | \hat{z} | u_{l}}\right].
\end{align}
对\(\Omega_{+}\)有
\begin{align}
	\mathrm{Re} \mathcal{F}^{+}_{\beta} = \mathcal{F}_{\beta}^{0} - \sum_{n \in O} \left(E_{l} + E_{n} - 2\mu\right) \mathrm{Re} \left[\braket{u_{n} | \partial_{k_{\beta}} u_{l}} \braket{u_{l } | \hat{z} | u_{n }}\right].
\end{align}
其中
\begin{align}
	\mathcal{F}_{\beta}^{0} = - \mathrm{Re} \sum_{\substack{n \in O \\ m \in E}} \left(E_{n } + E_{m } - 2 \mu\right) \mathrm{Re} \left[\braket{u_{n } | \partial_{k_{\beta}} u_{m }} \braket{u_{m } | \hat{z} | u_{n }}\right].
\end{align}
所以
\begin{align}
	\begin{split}
			\Delta_{l} \mathrm{Re} \mathcal{F}_{\beta} &= \mathrm{Re} \mathcal{F}^{-}_{\beta} - \mathrm{Re} \mathcal{F}^{+}_{\beta}	 \\
			& = - \sum_{m \in E} \left(E_{m} + E_{l} - 2\mu\right) \mathrm{Re} \left[\braket{u_{l} | \partial_{k_{\beta}} u_{m }} \braket{u_{m } | \hat{z} | u_{l}}\right] \\
			& \quad + \sum_{n \in O} \left(E_{l} + E_{n} - 2\mu\right) \mathrm{Re} \left[\braket{u_{n} | \partial_{k_{\beta}} u_{l}} \braket{u_{l } | \hat{z} | u_{n }}\right] .
	\end{split}
\end{align}
又
\begin{align}
	\mathrm{Re}\left[ \braket{u_{l} | \partial_{k_{\beta}} u_{m }} \braket{u_{m } | \hat{z} | u_{l}}\right] = - \mathrm{Re} \left[\bra{u_m | \partial_{k_{\beta}} u_{l}} \braket{u_{l} | \hat{z} | u_{m}}\right].
\end{align}
所以
\begin{align}
	\begin{split}
		\Delta_{l} \mathrm{Re} \mathcal{F}_{\beta} & = \sum_{n \in O} \left(E_n - \mu\right) \mathrm{Re} \left[\braket{u_n | \partial_{k_{\beta}} u_{l }} \braket{u_{l} | \hat{z} | u_{n}}\right] + \sum_{m \in E}  \left(E_m - \mu\right) \mathrm{Re} \left[\braket{u_m | \partial_{k_{\beta}} u_{l }} \braket{u_{l} | \hat{z} | u_{m}}\right], \\
		& = \sum_{r \neq l} \left(E_{r } - \mu\right) \mathrm{Re} \left[\braket{u_{r } | \partial_{k_{\beta}} u_{l }} \braket{u_{l} | \hat{z} | u_{r}}\right], \\
		& = - \sum_{r \neq l} \left(E_{r } - \mu\right) \mathrm{Re} \left[\braket{u_{l} | \partial_{k_{\beta}} u_{r}} \braket{u_{r} | \hat{z} | u_{l}}\right].
	\end{split}
\end{align}
写出对极化率的贡献为
\begin{align}
	\chi^{\mathrm{FS}}_{\alpha,2} = - \frac{e }{\hbar c } \epsilon_{\alpha \beta} \int_{\mathrm{mBZ}} \frac{\mathrm{d}^{2} \bm{k}}{\left(2 \pi\right)^2} \sum_{l} \sum_{r \neq l} \left(E_r - \mu\right) \delta \left(E_{l} - \mu\right) H_{\lambda,ll} \mathrm{Re} \left[\braket{u_{l} | \partial_{k_{\beta}} u_r} z_{rl}  \right],
\end{align}
将
\begin{align}
...
\begin{align}
	\boxed{
		\chi^{\mathrm{FS}}_{\alpha,2} = - \frac{e }{\hbar c } \epsilon_{\alpha \beta} \int_{\mathrm{mBZ}} \frac{\mathrm{d}^{2} \bm{k}}{\left(2 \pi\right)^2} \sum_{m} \sum_{n \neq m} \delta \left(E_{m} - \mu\right) \braket{u_m | \hat{H}_{\lambda} | u_{m} } \mathrm{Re} \left[\braket{u_{m} | \hat{H}_{\beta} | u_n} \braket{u_{n} | \hat{z} | u_{m}} \right],
	}
\end{align}
由此，极化率中来自费米面的贡献为
\begin{align}
	\begin{split}
		\boxed{
			\begin{aligned}
				& \chi^{\mathrm{FS}}_{\lambda, \alpha} = \frac{e }{\hbar c } \epsilon_{\alpha \beta} \int_{\mathrm{mBZ}} \frac{\mathrm{d}^{2} \bm{k}}{\left(2 \pi\right)^2} \sum_{m} \sum_{n \neq m} \delta \left(E_{m} - \mu\right) \\
		&  \left[\braket{u_{m} | \hat{H}_{\beta} | u_{m}} \mathrm{Re} \left(\braket{u_{m} | \hat{H}_{\lambda} | u_{n }} \braket{u_{n} | \hat{z} | u_{m }}\right) - \braket{u_m | \hat{H}_{\lambda} | u_{m} } \mathrm{Re} \left(\braket{u_{m} | \hat{H}_{\beta} | u_n} \braket{u_{n} | \hat{z} | u_{m}}\right) \right].
			\end{aligned}
		}
	\end{split}
\end{align}
...
对于占据态固定的两项\(\mathrm{Re} \mathrm{Tr} \left[\mathcal{P} \mathcal{P}_{\alpha} \mathcal{Q} \left( \partial_{\lambda} \mathcal{Z}\right) - \mathcal{P} \mathcal{P}_{\lambda} \mathcal{Q} \left(\partial_{k_{\alpha}} \mathcal{Z}\right)\right]\), 前面有
\begin{align}
...
由此
\begin{align}
	\mathrm{Re} \mathrm{Tr} \left[\mathcal{P} \mathcal{P}_{\alpha} \mathcal{Q} \left( \partial_{\lambda} \mathcal{Z}\right)\right]  &= \mathrm{Re} \sum_{\substack{n \in \mathcal{O}_s \\ m \notin \mathcal{O}_s}} \frac{H_{\alpha,nm } \left(\partial_{\lambda} \mathcal{Z}\right)_{mn}}{E_{n } - E_{m }}, \\
	-\mathrm{Re} \mathrm{Tr} \left[\mathcal{P} \mathcal{P}_{\lambda} \mathcal{Q} \left(\partial_{k_{\alpha}} \mathcal{Z}\right)\right] &= -\mathrm{Re}  \sum_{\substack{n \in \mathcal{O}_s \\ m \notin \mathcal{O}_s}} \frac{H_{\lambda,nm } \left(\partial_{\alpha} \mathcal{Z}\right)_{mn}}{E_{n } - E_{m }},
\end{align}
...
于是这一项的极化率贡献为
\begin{align}
	\chi^{\mathrm{FIX}}_{\lambda, \alpha} = - \frac{e}{ \hbar c } \epsilon_{\alpha \beta} \int_{\mathrm{mBZ}} \frac{\mathrm{d}^{2} \bm{k}}{\left(2 \pi\right)^2} \mathrm{Re} \sum_{n, m} \sum_{l} \frac{f_{n} \left(1 - f_{m }\right)}{E_{n } - E_{m }} \left[H_{\beta, nm} \mathcal{Z}_{\lambda,mn} - H_{\lambda, nm} \mathcal{Z}_{\beta,mn} \right]
\end{align}
全部写出
\begin{align}
	\begin{split}
		\boxed{
			\begin{aligned}
				\chi^{\mathrm{FIX}}_{\lambda, \alpha} &= - \frac{e}{ \hbar c } \epsilon_{\alpha \beta} \int_{\mathrm{mBZ}} \frac{\mathrm{d}^{2} \bm{k}}{\left(2 \pi\right)^2} \mathrm{Re} \sum_{n, m} \sum_{l} \frac{f_{n} \left(1 - f_{m }\right)}{E_{n } - E_{m }} \\
