...
\usepackage{booktabs, multirow, makecell,colortbl, enumitem, mathrsfs}
\usepackage{braket}
...
\begin{align}
	\partial_{\lambda} \mathcal{P}  = \sum_{n \in \mathrm{occ }} \left(\ket{\partial_{\lambda} u_{n }} \bra{u_{n }} + \ket{u_{n }} \bra{\partial_{\lambda} u_{n }}\right).
\end{align}
接下来的问题变成如何表示\(\ket{\partial_{\lambda} u_{n }} \)，在\(\ket{\partial_{\lambda} u_{n }}\)中插入完备性关系\(\mathcal{P} + \mathcal{Q}\) 
\begin{align}
	\partial_{\lambda} \mathcal{P} = \sum_{n \in \mathrm{occ}} \mathcal{P} \ket{\partial_{\lambda} u_{n }} \bra{u_{n }} + \mathcal{Q} \ket{\partial_{\lambda} u_{n }} \bra{u_{n }} + \ket{u_{n }} \bra{\partial_{\lambda} u_{n }} \mathcal{P} + \ket{u_{n }} \bra{\partial_{\lambda} u_{n }} \mathcal{Q}.
\end{align}
我们首先来看占据-占据的部分
\begin{align}
	\begin{split}
		\sum_{n \in \mathrm{occ}} \mathcal{P} \ket{\partial_{\lambda} u_{n }} \bra{u_{n }} + \ket{u_{n }} \bra{\partial_{\lambda} u_{n }} \mathcal{P} & = \sum_{n \in \mathrm{occ}} \sum_{l \in \mathrm{occ}} \ket{u_{l }} \braket{u_{l } | \partial_{\lambda} u_{n }} \bra{u_{n }} + \ket{u_{n }} \braket{\partial_{\lambda} u_{n } | u_{l }} \bra{u_{l }} \\
		& = \sum_{n \in \mathrm{occ}} \sum_{l \in \mathrm{occ}} \braket{u_{n } | u_{l }} \braket{u_l | \partial_{\lambda} u_{n }} + \braket{u_{l } | u_{n }} \braket{\partial_{\lambda} u_{n } | u_{l }} \\
		& = \sum_{n \in \mathrm{occ}} \sum_{l \in \mathrm{occ}} \delta_{n l } \left(\braket{u_l | \partial_{\lambda} u_{n }} + \braket{\partial_{\lambda} u_{n } | u_{l }}\right) \\
		& = \sum_{n \in \mathrm{occ}} \partial_{\lambda} \braket{u_{n } | u_{n }} \\
		& = 0.
	\end{split}
\end{align}
所以占据-占据对\(\partial_{\lambda} \mathcal{P}\)没有贡献，只剩下占据态-空态的部分
\begin{align}
	\partial_{\lambda} \mathcal{P} = \sum_{n \in \mathrm{occ}} \mathcal{Q} \ket{\partial_{\lambda} u_{n }} \bra{u_{n }}  + \ket{u_{n }} \bra{\partial_{\lambda} u_{n }} \mathcal{Q}.
\end{align}
进一步改写也就是
\begin{align}
	\boxed{
		\partial_{\lambda} \mathcal{P} = \mathcal{Q} \left(\partial_{\lambda} \mathcal{P}\right) \mathcal{P} + \mathcal{P} \left(\partial_{\lambda} \mathcal{P}\right) \mathcal{P}.	
	}
\end{align}
进一步把空态子空间的投影算符写出来，上式可以写成
\begin{align}
	\begin{split}
		\partial_{\lambda} \mathcal{P} & = \sum_{n \in \mathrm{occ}} \sum_{m \in \mathrm{emp}} \ket{u_{m }}\braket{u_{m} | \partial_{\lambda} u_{n }} \bra{u_{n }} + \ket{u_{n }} \braket{\partial_{\lambda} u_{n} | u_{m }} \bra{u_{m}} \\
		& = \sum_{n \in \mathrm{occ}} \sum_{m \in \mathrm{emp}} \ket{u_{m }} \frac{\braket{u_{m } | \partial_{\lambda} \hat{H} | u_{n }}}{E_{n } - E_{m }} \bra{u_{n }} + \ket{u_{n }} \frac{\braket{u_{n } | \partial_{\lambda} \hat{H} | u_{m}}}{E_{n } - E_{m }} \bra{u_{m}}.
	\end{split}
\end{align}
定义单带投影算符
\begin{align}
	\Pi_{n } = \ket{u_{n }} \bra{u_{n }}, \quad \Pi_{m } = \ket{u_{m }} \bra{u_{m }},
\end{align}
可以将上式改写成
\begin{align}
	\boxed{
		\partial_{\lambda} \mathcal{P} =  \sum_{n \in \mathrm{occ}} \sum_{m \in \mathrm{emp}} \frac{\Pi_{m } \left(\partial_{\lambda} \hat{H} \right)\Pi_{n } + \Pi_{n} \left(\partial_{\lambda} \hat{H} \right) \Pi_{m} }{E_{n } - E_{m }}.
	}
\end{align}
...
\begin{align}
	\boxed{
		M_{\alpha} = -\frac{e }{\hbar c } \epsilon_{\alpha \beta} \int_{\mathrm{mBZ }} \frac{\mathrm{d}^{2} \bm{k}}{\left(2 \pi\right)^2} \operatorname{Re} \mathcal{F}_{\beta} \left(\bm{k }\right).
	}
\end{align}
...
\subsection{极化率的表达式}
现在我们要将面内轨道磁化的表达式对应力参数\(\lambda\)进行求导。在此之前，首先说明我们考虑的是固定化学势的情况，即\(\partial_{\lambda} \mu = 0\)，同时应力也不改变上下层的层间距离，即\(\partial_{\lambda} \hat{z} = 0\)。为了符号简便，令\(\mathcal{P}_{\alpha} \equiv \partial_{k_{\alpha}} \mathcal{P }\)以及\(\mathcal{P}_{\lambda} \equiv \left(\partial \mathcal{P} / \partial \lambda \right)|_{\lambda = 0}\)。同时令\(\hat{H}_{\alpha} \equiv \partial \hat{H} / \partial_{k_{\alpha}}\)以及\(\hat{H}_{\lambda} \equiv \left(\partial \hat{H} / \partial \lambda\right)|_{\lambda = 0} \)。再次将面内轨道磁化强度的表达式写出
\begin{align}
		\boxed{
		M_{\alpha} = -\frac{e }{\hbar c } \epsilon_{\alpha \beta} \int_{\mathrm{mBZ }} \frac{\mathrm{d}^{2} \bm{k}}{\left(2 \pi\right)^2} \operatorname{Re} \, \mathrm{Tr} \left[\mathcal{P} \left(\partial_{k_{\beta}} \mathcal{P} \right) \mathcal{Q} \mathcal{Z}\right].
	}
\end{align}
...
所以面内轨道磁化的表达式需要改写成
\begin{align}
	M_{\alpha} = -\frac{e }{\hbar c} \epsilon_{\alpha \beta} \sum_{s } \int_{\Omega_{s }} \frac{\mathrm{d}^{2} \bm{k}}{\left(2 \pi\right)^2} \operatorname{Re} \, \mathcal{F}^{\left(s \right)}_{\beta} \left(\bm{k }\right),
\end{align}
其中
\begin{align}
...
\begin{align}
	\frac{\partial}{\partial \lambda} \int_{\Omega_{s }} \frac{\mathrm{d}^{2} \bm{k}}{\left(2 \pi\right)^2} \mathcal{F} \left(\bm{k }\right) = \int_{\Omega_{s }} \frac{\mathrm{d}^{2} \bm{k}}{\left(2 \pi\right)^2} \partial \lambda \mathcal{F}  \left(\bm{k }\right) + \oint_{\partial \Omega_{s }} \frac{\mathrm{d} \ell}{\left(2 \pi\right)^{2}} v_{\lambda} \mathcal{F} \left(\bm{k}\right).
\end{align}
...
\end{align}
展开的第二项正是我们所需要的，将起代入到 \(\partial_{\lambda} \mathcal{F} \left(\bm{k }\right)\) ，得到
\begin{align}
	\begin{split}
...
\begin{align}
	...
	\end{split}
\end{align}
这里由于投影算符的导数满足\(\mathcal{P} \mathcal{P}_{\lambda/\alpha} \mathcal{P} = \mathcal{Q} \mathcal{P}_{\lambda/\alpha} \mathcal{Q} = 0\)，故\(\mathcal{P}_{\lambda/\alpha}\)和\(\mathcal{Q}_{\lambda/\alpha}\)仅含\(\mathcal{P}-\mathcal{Q}\)非对角分量，从而其对易子\(\left[\mathcal{P}_{\lambda}, \mathcal{P}_{\alpha}\right]\)在\(\mathcal{P}/\mathcal{Q}\)空间中是分块对角的，即与\(\mathcal{P}\)对易。因此
\begin{align}
	\left[\mathcal{P}_{\lambda}, \mathcal{P}_{\alpha}\right] \mathcal{Q} + \mathcal{P} \left[\mathcal{P}_{\lambda}, \mathcal{P}_{\alpha}\right] = \left[\mathcal{P}_{\lambda}, \mathcal{P}_{\alpha}\right] \left(\mathcal{Q} + \mathcal{P}\right) = \left[\mathcal{P}_{\lambda}, \mathcal{P}_{\alpha}\right].
\end{align}
由于\(\mathcal{P}_{\lambda}^{\dagger} = \mathcal{P}_{\lambda}, \quad \mathcal{P}_{\alpha}^{\dagger} = \mathcal{P}_{\alpha}\), 所以
\begin{align}
	\left[\mathcal{P}_{\lambda}, \mathcal{P}_{\alpha}\right]^{\dagger} = - \left[\mathcal{P}_{\lambda}, \mathcal{P}_{\alpha}\right],
\end{align}
这意味着 \(\left[\mathcal{P}_{\lambda}, \mathcal{P}_{\alpha}\right] \)是反厄米的。同时 \(\mathcal{Z}^{\dagger} = \mathcal{Z}\), 所以 \(\mathrm{Tr} \left[\left[\mathcal{P}_{\lambda}, \mathcal{P}_{\alpha}\right] \mathcal{Z}\right]\)是纯虚数. 因为面内轨道磁化强度是取\(\partial_{\lambda} \mathcal{F}\)的实部，所以这一项对面内轨道磁化没有贡献。即,
\begin{align}
	\operatorname{Re} \, \mathrm{Tr} \left[ \left[\mathcal{P}_{\lambda}, \mathcal{P}_{\alpha}\right]  \mathcal{Z} \right] = 0.
\end{align}
由此可以得到 
\begin{align}
	\boxed{
		\operatorname{Re} \, \partial_{\lambda} \mathcal{F}_{\alpha} = \partial_{k_{\alpha}} \operatorname{Re} \mathrm{Tr} \left[\mathcal{P} \mathcal{P}_{\lambda} \mathcal{Q} \mathcal{Z} \right] + \operatorname{Re} \mathrm{Tr} \left[\mathcal{P} \mathcal{P}_{\alpha} \mathcal{Q} \left(\partial_{\lambda} \mathcal{Z}\right) - \mathcal{P} \mathcal{P}_{\lambda} \mathcal{Q} \left(\partial_{k_{\alpha}} \mathcal{Z}\right) \right].
	}
\end{align}
我们首先处理第一项，第一项的积分是
\begin{align}
	I^{1}_{\alpha}  = \sum_{s} \int_{\Omega_{s }} \frac{\mathrm{d}^{2} \bm{k}}{\left(2 \pi\right)^2}  \partial_{k_{\alpha}} \operatorname{Re} \mathrm{Tr} \left[\mathcal{P} \mathcal{P}_{\lambda} \mathcal{Q} \mathcal{Z} \right] 
\end{align}
二维高斯定理给出
\begin{align}
	\int_{\Omega_{s}} \mathrm{d}^{2} \bm{k}	\bm{\nabla}_{\bm{k}} \cdot \bm{A}_{s} = \oint_{\partial \Omega_{s}} \mathrm{d} l \hat{\bm{n}}_{s} \cdot \bm{A}_{s},
\end{align}
因此
\begin{align}
	\int_{\Omega_{s}} \mathrm{d}^{2} k \partial_{k_{\alpha}} A_{s } = \oint_{\partial \Omega_{s}} \mathrm{d} \ell \hat{\bm{n}}_{s} \cdot \left(A_{s } \hat{\bm{e}}_{\alpha}\right) =  \oint_{\partial \Omega_{s}} \mathrm{d} \ell n_{s, \alpha} A_{s}.
\end{align}
其中
\begin{align}
	n_{s,\alpha} \equiv \hat{\bm{n }}_{s } \cdot \hat{\bm{e}}_{\alpha},
\end{align}
是区域\(\Omega_{s }\)的边界\(\partial \Omega_{s}\)上，向外单位法向量的第\(\alpha\)个分量。为了简化符号，我们定义
\begin{align}
	\mathcal{A} = \operatorname{Re} \mathrm{Tr} \left[\mathcal{P} \mathcal{P}_{\lambda} \mathcal{Q} \mathcal{Z} \right] .
\end{align}
由此可以将第一项的求和积分写成
\begin{align}
	I^{1}_{\alpha}  = \sum_{s } \oint_{\partial \Omega_{s }} \frac{\mathrm{d} \ell }{\left(2 \pi\right)^2} n_{s,\alpha} \mathcal{A}_{s}.
\end{align}
现在，假设某条费米面是由\(\mu\)穿过\(E_{l} \left(\bm{k}\right)\)所得到的，费米面的两侧分别是\(E_{l} < \mu\)和\(E_{l} > \mu\), 定义法向量
\begin{align}
	\hat{\bm{n }}_{l} = \frac{\nabla_{\bm{k }} E_{l}}{\left|\nabla_{\bm{k }} E_{l}\right|}
\end{align}
法向量的指向为从\(E_{l } < \mu\)一侧，指向\(E_{l} > \mu\)一侧。那么，法向量的分量为
\begin{align}
	n_{l, \alpha} = \frac{\partial_{k_{\alpha}} E_{l}}{\left|\nabla_{\bm{k }} E_{l}\right|}.
\end{align}
假设费米面两侧对应的两个区域分别为: \(\Omega_{-}: \, E_{l} < \mu\) 和\(\Omega_{+} : \, E_{l} > \mu\)，那么对于\(\Omega_{-}\)，其对应费米面上的法向量为\(\hat{\bm{n }}_{-} = \hat{\bm{n }}_{l}\)，对于\(\Omega_{+}\), 其对应费米面上的法向量为\(\hat{\bm{n}}_{+} = - \hat{\bm{n}}_{l}\)，由此积分变为
\begin{align}
	\oint_{\partial \Omega_{s }} \mathrm{d} \ell n_{-, \alpha} \mathcal{A}_{-} + \oint_{\partial \Omega_{s }} \mathrm{d} \ell n_{+, \alpha} \mathcal{A}_{+} = \oint_{\partial \Omega_{s }} \mathrm{d} \ell \, n_{l, \alpha} \left(\mathcal{A}_{-} - \mathcal{A}_{+}\right).
\end{align}
这里我们定义穿过第\(l\)条能带费米面的jump	
\begin{align}
	\Delta_{l } \mathcal{A} \equiv \mathcal{A}_{-} - \mathcal{A}_{+} = \mathcal{A}_{l}^{\mathrm{occ}} - \mathcal{A}_{l}^{\mathrm{emp}}.
\end{align}
所以对于固定秩投影算符的二维区域的求和，变成了对穿过化学势的能带求和（但是这里的积分仍然是对费米面的积分（或者是二维区域的边界进行线积分），(这里需要指出严格意义上的\(\partial \Omega_{s}\)包括布里渊区的边界，但是布里渊区边界的贡献由于平移对称性抵消掉了)。将上式进行改写
\begin{align}
	\sum_{s} \oint_{\partial \Omega_{s}} \mathrm{d} \ell n_{s, \beta} \mathcal{A}_{s }	= \sum_{l} \oint_{E_{l} = \mu} \mathrm{d} \ell n_{l, \beta} \Delta_{l} \mathcal{A}.
\end{align}
于是第一项的积分可以重新写成
\begin{align}
	I_{1, \alpha} = \sum_{l} \oint_{E_{l} = \mu} \frac{\mathrm{d} \ell }{\left(2 \pi\right)^{2}} \frac{\partial_{k_{\alpha}} E_{l}}{\left|\nabla_{\bm{k }} E_{l }\right|} \Delta_{l} \mathcal{A}.
\end{align}
由此可以将线积分写成对（莫尔）布里渊区的面积分
\begin{align}
	\int_{\mathrm{mBZ}} \mathrm{d}^{2} \bm{k} \delta \left( E_{l} - \mu\right) \partial_{k_{\alpha}} E_{l}  \Delta_{l} \mathcal{A} = \oint_{E_{l} = \mu} \frac{\mathrm{d} \ell}{\left|\nabla_{\bm{k }} E_{l}\right|} \left(\partial_{k_{\alpha}} E_{l} \right) \Delta_{l} \mathcal{A}.
\end{align}
就可以得到
\begin{align}
	I^{1}_{\alpha}  = \int_{\mathrm{mBZ}} \frac{\mathrm{d}^{2} \bm{k}}{\left(2 \pi\right)^{2}} \sum_{l} \delta \left( E_{l} - \mu\right) \left( \partial_{k_{\alpha}} E_{l} \right) \Delta_{l} \left\{\operatorname{Re} \mathrm{Tr} \left[\mathcal{P} \mathcal{P}_{\lambda} \mathcal{Q} \mathcal{Z} \right] \right\}.  
\end{align}
现在我们来处理\(\mathcal{P}_{\lambda}\)，前面有
\begin{align}
	\mathcal{P}_{\lambda} = \sum_{\substack{n \in \mathrm{occ} \\ m \in \mathrm{emp}}} \frac{\Pi_{n } \hat{H}_{\lambda} \Pi_{m } + \Pi_{m} \hat{H}_{\lambda} \Pi_{n}}{E_{n } - E_{m }},
\end{align}
显然有
\begin{align}
	\mathcal{P} \mathcal{P}_{\lambda} \mathcal{Q} = \sum_{\substack{n \in \mathrm{occ} \\ m \in \mathrm{emp}}} \frac{\Pi_{n } \hat{H}_{\lambda} \Pi_{m }}{E_{n } - E_{m }}.
\end{align}
所以
\begin{align}
	\mathcal{A} = \operatorname{Re} \sum_{\substack{n \in \mathrm{occ} \\ m \in \mathrm{emp}}} \frac{\mathrm{Tr} \left[\Pi_{n } \hat{H}_{\lambda} \Pi_{m } \mathcal{Z}\right]}{E_{n} - E_{m}} = \operatorname{Re} \sum_{\substack{n \in \mathrm{occ} \\ m \in \mathrm{emp}}} \frac{H_{\lambda,nm} \mathcal{Z}_{mn} }{E_{n } - E_{m }}.
\end{align}
这里暂时不考虑\(E_{n} = E_{m }\)的情况。其中
\begin{align}
	H_{\lambda, nm} & \equiv \braket{u_{n} | \hat{H}_{\lambda} | u_{m }}, \\
	\mathcal{Z}_{mn} & \equiv \braket{u_{m } | \mathcal{Z} | u_{n }} = \left(E_{n} + E_{m } - 2 \mu\right) \braket{u_{m } | \hat{z} | u_{n }} , \\
	z_{mn}& = \braket{u_{m } | \hat{z} | u_{n }} .
\end{align}
进一步将\(\mathcal{A}\)写成
\begin{align}
	\mathcal{A} = \operatorname{Re} \sum_{\substack{n \in \mathrm{occ} \\ m \in \mathrm{emp}}} \frac{E_{n} + E_{m } - 2 \mu}{E_{n } - E_{m }} H_{\lambda, nm} z_{mn}.
\end{align}
现在再次考虑\(\Omega_{\pm}\)两个区域，定义\(O\)是完全占据的能带集合，\(E\)是完全空的能带集合。这意味着，对于区域\(\Omega_{-}\)中的\(\bm{k}\)点，其对应占据的能带集合是\(O \cup \left\{l\right\}\)，空能带集合是\(E\)；对于区域对于区域\(\Omega_{+}\)中的\(\bm{k}\)点，其对应占据的能带集合是\(O\)，空能带集合是\(E \cup \left\{l\right\}\)。由此，我们可以写出两个区域内的\(\mathcal{A}_{\pm}\). 首先, 在\(\Omega_{-}\)
\begin{align}
	\begin{split}
		\mathcal{A}_{-}  = \operatorname{Re} \left[\sum_{n \in O} \sum_{m \in E} + \sum_{n =l} \sum_{m \in E}\right] = \mathcal{A}_{0} + \operatorname{Re} \sum_{m \in E} \frac{E_{l} + E_{m } - 2 \mu}{E_{l} - E_{m }} H_{\lambda,lm} z_{ml} ,
	\end{split}
\end{align}
其中
\begin{align}
	\mathcal{A}_{0} = \operatorname{Re} \sum_{n \in O} \sum_{m \in E} \frac{E_{n} + E_{m } - 2 \mu}{E_{n } - E_{m }} H_{\lambda, nm} z_{mn}.
\end{align}
那么在\(\Omega_{+}\)中，则有
\begin{align}
	\begin{split}
		\mathcal{A}_{+} =  \operatorname{Re} \left[\sum_{n \in O} \sum_{m \in E} + \sum_{n \in O} \sum_{m = l}\right] = \mathcal{A}_{0} + \operatorname{Re} \sum_{n \in O } \frac{E_{n} + E_{l} - 2 \mu}{E_{n } - E_{l}} H_{\lambda,nl} z_{ln}.
	\end{split}
\end{align}
由于我们只是在\(\partial \Omega\)这样的费米面上考虑，所以取\(E_{l} = \mu\), 代入有
\begin{align}
	\frac{E_{m } + E_{l } - 2 \mu }{E_{l } - E_{m }} = \frac{ E_{m } + \mu - 2 \mu}{\mu - E_{m }} = \frac{E_{m } - \mu}{\mu - E_{m }} = -1,
\end{align}
所以
\begin{align}
	\mathcal{A}_{-} = \mathcal{A}_{0} - \operatorname{Re}  \sum_{m \in E } H^{\lambda}_{lm } z_{m l}.
\end{align}
同理对于\(\mathcal{A}_{- }\),
\begin{align} 
	\frac{E_{l } + E_{n } - 2 \mu}{E_{n } - E_{l}} = \frac{\mu + E_{n } - 2 \mu}{E_{n } - \mu} = \frac{E_{n } - \mu}{E_{n } - \mu} = 1.
\end{align}
所以
\begin{align}
	\mathcal{A}_{+} = \mathcal{A}_{0} + \operatorname{Re} \sum_{n \in O } H_{\lambda,nl} z_{ln}.
\end{align}
两者的差为
\begin{align}
	\mathcal{A}_{-} - \mathcal{A}_{+} = - \operatorname{Re} \sum_{m \in E } H_{\lambda,lm} z_{m l} - \operatorname{Re} \sum_{n \in O } H_{\lambda,nl} z_{ln},
\end{align}
由于\(\hat{H}_{\lambda}\)和\( \hat{z} \)都是厄米的，所以有 \(H_{\lambda,nl} = \left(H_{\lambda,ln}\right)^{*}\)，同时\(z_{ln} = z^{*}_{nl}\), 所以
\begin{align}
	\operatorname{Re} H_{\lambda,nl} z_{ln} = \operatorname{Re} \left( H_{\lambda,ln} z_{nl} \right)^{*} = \operatorname{Re} H^{\lambda}_{ln} z_{nl},
\end{align}
可以将上式改写成
\begin{align}
	\mathcal{A}_{-} - \mathcal{A}_{+} = - \operatorname{Re} \sum_{m \in E}  H_{\lambda,lm} z_{ml} - \operatorname{Re} \sum_{n \in O } H_{\lambda, ln} z_{nl}  = - \operatorname{Re} \sum_{r \neq l} H_{\lambda, lr} z_{rl}
\end{align}
这里我们使用了一个虚拟变量\(r\)，为了符合我们的使用习惯，我们做指标替换\(r \rightarrow n\)和\(l \rightarrow m\)。再次说明的是，\(m\)与费米面相交的能带指标，而\(n\)是其他能带指标。因此，积分为
\begin{align}
	I^{1}_{\alpha} = - \int_{\mathrm{mBZ}} \frac{\mathrm{d}^{2} \bm{k}}{\left(2 \pi\right)^2} \sum_{m} \delta \left(E_{m} - \mu\right) \left(\partial_{k_{\alpha}} E_{m }\right) \sum_{n \neq m } \operatorname{Re} \left[H_{\lambda,mn} z_{nm}\right]，
\end{align}
这里\(\partial_{k_{\alpha}} E_{m } = \braket{u_{m } | \hat{H}_{\alpha} | u_{m}}\)。费米面对极化率的第一项贡献为
\begin{align}
	\boxed{
		\chi^{\mathrm{FS}, 1}_{\alpha} = \frac{e}{\hbar c} \epsilon_{\alpha \beta} \int_{\mathrm{mBZ}} \frac{\mathrm{d}^{2} \bm{k}}{\left(2 \pi\right)^2} \sum_{m} \delta \left(E_{m } - \mu\right) \braket{u_{m} | \hat{H}_{\beta} | u_{m}} \sum_{n \neq m} \operatorname{Re} \left[ \braket{u_{m} | \hat{H}_{\lambda} | u_{n }} \braket{u_{n} | \hat{z} | u_{m }}\right].
	}
\end{align}
这里\(m\)是与化学势相交的能带指标。
...
现在，我们来看费米面对极化率的第二项贡献，也就是
\begin{align}
	\chi^{\mathrm{FS},2}_{\alpha} = -\frac{e}{\hbar c} \epsilon_{\alpha \beta} \sum_{s } \oint_{\partial \Omega_{s }} \frac{ \mathrm{d} \ell}{\left(2 \pi\right)^{2} } \left(\frac{\partial \bm{k}_{\mathrm{FS}}}{\partial \lambda} \cdot \hat{\bm{n }}_{s}\right) \operatorname{Re} \mathcal{F}_{\beta}^{s}.
\end{align}
定义费米面移动速度
\begin{align}
	v_{\lambda} \equiv \frac{\partial \bm{k}_{\mathrm{FS}}}{\partial \lambda} \cdot \hat{\bm{n}},
\end{align}
同理，考虑\(E_{l} = \mu\)的费米面\(\partial \Omega\)两侧的\(\Omega_{\pm}\)，其在极化率中的贡献可以被类似的写出
\begin{align}
	\left(\frac{\partial \bm{k}_{\mathrm{FS}}}{\partial \lambda} \cdot \hat{\bm{n}}_{-}\right) \operatorname{Re} \mathcal{F}^{-}_{\beta} + \left(\frac{\partial \bm{k}_{\mathrm{FS}}}{\partial \lambda} \cdot \hat{\bm{n}}_{+}\right) \operatorname{Re} \mathcal{F}^{+}_{\beta} = v_{l,\lambda} \left[\operatorname{Re} \mathcal{F}^{-}_{\beta} - \operatorname{Re} \mathcal{F}^{+}_{\beta}\right].
\end{align}
现在的问题在于\(v_{l,\lambda}\)如何求解，考虑\(E_{l} \left(\bm{k}_{\mathrm{FS}} \left(\lambda\right), \lambda\right) = \mu\), 两边对\(\lambda\)求导得到
\begin{align}
	\begin{split}
		\quad \partial_{\lambda} E_{\lambda} + \nabla_{\bm{k}} E_{l} \cdot \frac{\partial \bm{k}_{\mathrm{FS}}}{\partial \lambda} & = 0 \\
		\Rightarrow \quad \nabla_{\bm{k}} E_{l } \cdot \frac{\partial \bm{k}_{\mathrm{FS}}}{\partial \lambda} & = - \partial_{\lambda} E_{l }	= - H_{\lambda,ll},
	\end{split}
\end{align}
又有\(\nabla_{\bm{k }} E_{l } = \left|\nabla_{\bm{k }} E_{l } \right| \hat{\bm{n }}_{l}\), 由此
\begin{align}
	\begin{split}
		\left|\nabla_{\bm{k }} E_{l } \right| \hat{\bm{n }}_{l} \cdot \frac{\partial \bm{k}_{\mathrm{FS}}}{\partial \lambda} &= - H_{\lambda,ll} \\
		\Rightarrow \quad  v_{l,\lambda} &=  - \frac{ H_{\lambda,ll}}{\left|\nabla_{\bm{k}} E_{l}\right|}.
	\end{split}
\end{align}
定义
\begin{align}
	\Delta_{l} \operatorname{Re} \mathcal{F}_{\beta} \equiv \operatorname{Re} \left[\mathcal{F}^{-}_{\beta} - \mathcal{F}^{+}_{\beta}\right].
\end{align}
可以写出费米面对极化率的第二项贡献为
\begin{align}
	\chi^{\mathrm{FS},2}_{\alpha} =  \frac{e }{\hbar c} \epsilon_{\alpha \beta} \sum_{l} \oint_{E_{l} = \mu} \frac{\mathrm{d} \ell}{ \left(2 \pi\right)^2} \frac{ H_{\lambda,ll}}{\left|\nabla_{\bm{k}} E_{l }\right|} \Delta_{l } \operatorname{Re} \mathcal{F}_{\beta},
\end{align}
将线积分改写成面积分
\begin{align}
	\chi^{\mathrm{FS},2}_{\alpha} =  \frac{e }{\hbar c} \epsilon_{\alpha \beta} \sum_{l} \int_{\mathrm{mBZ}} \frac{\mathrm{d}^{2} \bm{k}}{\left(2 \pi\right)^2} \delta \left(E_{l} - \mu\right) H_{\lambda,ll} \Delta_{l} \operatorname{Re} \mathcal{F}_{\beta}.
\end{align}
现在来处理\(\Delta_{l} \operatorname{Re} \mathcal{F}_{\beta}\), 由于
\begin{align}
	\operatorname{Re} \mathcal{F}_{\beta} = -\sum_{\substack{n \in \mathrm{occ} \\ m \in \mathrm{emp}}} \left(E_{n } + E_{m } - 2\mu\right) \operatorname{Re} \left[\braket{u_{n } | \partial_{k_{\beta}} u_{m }} \braket{u_{m } | \hat{z} | u_{n }}\right].
\end{align}
对\(\Omega_{-}\)这一侧区域有
\begin{align}
	\operatorname{Re} \mathcal{F}^{-}_{\beta} = \mathcal{F}_{\beta}^{0} - \sum_{m \in E} \left(E_{m} + E_{l} - 2\mu\right) \operatorname{Re} \left[\braket{u_{l} | \partial_{k_{\beta}} u_{m }} \braket{u_{m } | \hat{z} | u_{l}}\right].
\end{align}
对\(\Omega_{+}\)有
\begin{align}
	\operatorname{Re} \mathcal{F}^{+}_{\beta} = \mathcal{F}_{\beta}^{0} - \sum_{n \in O} \left(E_{l} + E_{n} - 2\mu\right) \operatorname{Re} \left[\braket{u_{n} | \partial_{k_{\beta}} u_{l}} \braket{u_{l } | \hat{z} | u_{n }}\right].
\end{align}
其中
\begin{align}
	\mathcal{F}_{\beta}^{0} = - \operatorname{Re} \sum_{\substack{n \in O \\ m \in E}} \left(E_{n } + E_{m } - 2 \mu\right) \operatorname{Re} \left[\braket{u_{n } | \partial_{k_{\beta}} u_{m }} \braket{u_{m } | \hat{z} | u_{n }}\right].
\end{align}
所以
\begin{align}
	\begin{split}
			\Delta_{l} \operatorname{Re} \mathcal{F}_{\beta} &= \operatorname{Re} \mathcal{F}^{-}_{\beta} - \operatorname{Re} \mathcal{F}^{+}_{\beta}	 \\
			& = - \sum_{m \in E} \left(E_{m} + E_{l} - 2\mu\right) \operatorname{Re} \left[\braket{u_{l} | \partial_{k_{\beta}} u_{m }} \braket{u_{m } | \hat{z} | u_{l}}\right] \\
			& \quad + \sum_{n \in O} \left(E_{l} + E_{n} - 2\mu\right) \operatorname{Re} \left[\braket{u_{n} | \partial_{k_{\beta}} u_{l}} \braket{u_{l } | \hat{z} | u_{n }}\right] .
	\end{split}
\end{align}
又
\begin{align}
	\operatorname{Re}\left[ \braket{u_{l} | \partial_{k_{\beta}} u_{m }} \braket{u_{m } | \hat{z} | u_{l}}\right] = - \operatorname{Re} \left[\bra{u_m | \partial_{k_{\beta}} u_{l}} \braket{u_{l} | \hat{z} | u_{m}}\right].
\end{align}
所以
\begin{align}
	\begin{split}
		\Delta_{l} \operatorname{Re} \mathcal{F}_{\beta} & = \sum_{n \in O} \left(E_n - \mu\right) \operatorname{Re} \left[\braket{u_n | \partial_{k_{\beta}} u_{l }} \braket{u_{l} | \hat{z} | u_{n}}\right] + \sum_{m \in E}  \left(E_m - \mu\right) \operatorname{Re} \left[\braket{u_m | \partial_{k_{\beta}} u_{l }} \braket{u_{l} | \hat{z} | u_{m}}\right], \\
		& = \sum_{r \neq l} \left(E_{r } - \mu\right) \operatorname{Re} \left[\braket{u_{r } | \partial_{k_{\beta}} u_{l }} \braket{u_{l} | \hat{z} | u_{r}}\right], \\
		& = - \sum_{r \neq l} \left(E_{r } - \mu\right) \operatorname{Re} \left[\braket{u_{l} | \partial_{k_{\beta}} u_{r}} \braket{u_{r} | \hat{z} | u_{l}}\right].
	\end{split}
\end{align}
写出对极化率的贡献为
\begin{align}
	\chi^{\mathrm{FS},2}_{\alpha} = - \frac{e }{\hbar c } \epsilon_{\alpha \beta} \int_{\mathrm{mBZ}} \frac{\mathrm{d}^{2} \bm{k}}{\left(2 \pi\right)^2} \sum_{l} \left(E_r - \mu\right) \delta \left(E_{l} - \mu\right) H_{\lambda,ll} \sum_{r \neq l}  \operatorname{Re} \left[\braket{u_{l} | \partial_{k_{\beta}} u_r} z_{rl}  \right],
\end{align}
将
\begin{align}
	\braket{u_{l} | \partial_{k_{\beta}} u_r} = \frac{\braket{u_l | \hat{H}_{\beta} | u_r }}{E_r - E_l} = \frac{\braket{u_l | \hat{H}_{\beta}| u_r}}{E_r - \mu}.
\end{align}
代入得到
\begin{align}
	\boxed{
		\chi^{\mathrm{FS},2}_{\alpha} = - \frac{e }{\hbar c } \epsilon_{\alpha \beta} \int_{\mathrm{mBZ}} \frac{\mathrm{d}^{2} \bm{k}}{\left(2 \pi\right)^2} \sum_{m} \delta \left(E_{m} - \mu\right) \braket{u_m | \hat{H}_{\lambda} | u_{m} } \sum_{n \neq m} \operatorname{Re} \left[\braket{u_{m} | \hat{H}_{\beta} | u_n} \braket{u_{n} | \hat{z} | u_{m}} \right],
	}
\end{align}
由此，极化率中来自费米面的贡献为
\begin{align}
	\begin{split}
		\boxed{
			\begin{aligned}
				& \chi^{\mathrm{FS}}_{\lambda, \alpha} = \frac{e }{\hbar c } \epsilon_{\alpha \beta} \int_{\mathrm{mBZ}} \frac{\mathrm{d}^{2} \bm{k}}{\left(2 \pi\right)^2} \sum_{m} \delta \left(E_{m} - \mu\right) \\
		& \sum_{n \neq m} \left[\braket{u_{m} | \hat{H}_{\beta} | u_{m}} \operatorname{Re} \left(\braket{u_{m} | \hat{H}_{\lambda} | u_{n }} \braket{u_{n} | \hat{z} | u_{m }}\right) - \braket{u_m | \hat{H}_{\lambda} | u_{m} } \operatorname{Re} \left(\braket{u_{m} | \hat{H}_{\beta} | u_n} \braket{u_{n} | \hat{z} | u_{m}}\right) \right].
			\end{aligned}
		}
	\end{split}
\end{align}
...
对于占据态固定的两项\(\operatorname{Re} \mathrm{Tr} \left[\mathcal{P} \mathcal{P}_{\alpha} \mathcal{Q} \left( \partial_{\lambda} \mathcal{Z}\right) - \mathcal{P} \mathcal{P}_{\lambda} \mathcal{Q} \left(\partial_{k_{\alpha}} \mathcal{Z}\right)\right]\), 前面有
\begin{align}
	\mathcal{P} \mathcal{P}_{\lambda/\alpha} \mathcal{Q} = \sum_{\substack{n \in \mathcal{O}_s \\ m \notin \mathcal{O}_s}} \frac{\Pi_{n} \hat{H}_{\lambda/\alpha} \Pi_{m }}{E_{n } - E_{m }}.
\end{align}
这里我们把求和做了替换, 替换成了\(\Omega_{s }\)对应的集合
\begin{align}
	\sum_{\substack{n \in \mathrm{occ} \\ m \in \mathrm{emp}}} \rightarrow \sum_{\substack{n \in \mathcal{O}_s \\ m \notin \mathcal{O}_s}},
\end{align}
由此
\begin{align}
	\operatorname{Re} \mathrm{Tr} \left[\mathcal{P} \mathcal{P}_{\alpha} \mathcal{Q} \left( \partial_{\lambda} \mathcal{Z}\right)\right]  &= \operatorname{Re} \sum_{\substack{n \in \mathcal{O}_s \\ m \notin \mathcal{O}_s}} \frac{H_{\alpha,nm } \left(\partial_{\lambda} \mathcal{Z}\right)_{mn}}{E_{n } - E_{m }}, \\
	-\operatorname{Re} \mathrm{Tr} \left[\mathcal{P} \mathcal{P}_{\lambda} \mathcal{Q} \left(\partial_{k_{\alpha}} \mathcal{Z}\right)\right] &= -\operatorname{Re}  \sum_{\substack{n \in \mathcal{O}_s \\ m \notin \mathcal{O}_s}} \frac{H_{\lambda,nm } \left(\partial_{\alpha} \mathcal{Z}\right)_{mn}}{E_{n } - E_{m }},
\end{align}
\(T = 0\)时，用分布函数\(f_{n} = \Theta \left(\mu - E_{n}\right)\)对求和做替换，同时对所有的\(\Omega_{s }\)做求和，等价于对（莫尔）布里渊区做积分
\begin{align}
	\sum_{\substack{n \in \mathcal{O}_s \\ m \notin \mathcal{O}_s}} \rightarrow \sum_{m,n} f_{n} \left(1 - f_{m}\right),\quad \sum_{s } \int_{\Omega_{s }} \rightarrow \int_{\mathrm{mBZ}},
\end{align}
于是这一项的极化率贡献为
\begin{align}
	\boxed{
		\chi^{\mathrm{FIX}}_{\lambda, \alpha} = - \frac{e}{ \hbar c } \epsilon_{\alpha \beta} \int_{\mathrm{mBZ}} \frac{\mathrm{d}^{2} \bm{k}}{\left(2 \pi\right)^2} \operatorname{Re} \sum_{n \neq m} \frac{f_{n} \left(1 - f_{m }\right)}{E_{n } - E_{m}} \left[H_{\beta, nm} \mathcal{Z}_{\lambda,mn} - H_{\lambda, nm} \mathcal{Z}_{\beta,mn} \right]
	}
\end{align}
全部写出
\begin{align}
	\begin{split}
		\boxed{
			\begin{aligned}
				\chi^{\mathrm{FIX}}_{\lambda, \alpha} &= - \frac{e}{ \hbar c } \epsilon_{\alpha \beta} \int_{\mathrm{mBZ}} \frac{\mathrm{d}^{2} \bm{k}}{\left(2 \pi\right)^2} \operatorname{Re} \sum_{n \neq m} \sum_{l} \frac{f_{n} \left(1 - f_{m }\right)}{E_{n } - E_{m }} \\
				& \quad \left[ \braket{u_{n} | \hat{H}_{\beta} | u_{m }} \left(\braket{u_{m} | \hat{z} | u_{l}} \braket{u_{l} | \hat{H}_{\lambda} | u_{n }} + \braket{u_{m} | \hat{H}_{\lambda}| u_{l}} \braket{u_{l} | \hat{z} | u_{n }}\right) \right. \\
				& \left. \quad - \braket{u_{n} | \hat{H}_{\lambda} | u_{m }} \left(\braket{u_{m} | \hat{z} | u_{l}} \braket{u_{l} | \hat{H}_{\beta} | u_{n}} + \braket{u_{m} | \hat{H}_{\beta}| u_{l}} \braket{u_{l} | \hat{z} | u_{n }}\right)
				\right].
			\end{aligned}
		}
	\end{split}
...
总的磁化率（压磁系数）就等于
\begin{align}
	\chi^{\mathrm{TOT}}_{\lambda, \alpha} = \chi^{\mathrm{FIX}}_{\lambda, \alpha} + \chi^{\mathrm{FS}}_{\lambda, \alpha}.
\end{align}
这里面的问题在于，公式的适用条件为\(E_{n } \neq E_{m }\)和\(\left|\nabla_{\bm{k}} E_{m}\right| \neq 0\), 对于狄拉克点这样的简并点，非简并微扰公式失效了，这通常对应\(n \neq m, \, E_{n} = E_{m}\)。但通常来说，狄拉克点并不会对积分有贡献，因此可以忽略掉。但是，对于范霍夫奇点，并不能这么简单的忽略。


\section{Toy Model 数值检验}
在具体阐述toy model的数值结果之前，首先申明我们的数值计算工作流程：在我们的程序当中，我们使用了python 和 fortran 两种语言来对轨道磁化强度和极化率进行计算。python 负责快速计算出结果，对于每一个model都会有一个脚本进行计算，输出和绘图流程，不采用并行优化；fortran 负责写出一般性的能带结构、轨道磁化、极化率的计算模块，并采用并行优化。当两者的结果一致，才对结果进一步分析。原始数据请见github对应仓库。

\subsection{Bilayer QWZ model}
对于我们的toy model, 我们选择bilayer QWZ model。首先，如果需要产生面内轨道磁化，需要时间反演对称性破缺。而当陈数不为零时，时间反演对称性一定破缺。因此，最简单的选择，就是陈绝缘体。考虑一个单层的 QWZ model, 其哈密顿量为
\begin{align} 
	h_{\mathrm{QWZ}} \left(\bm{k}\right) = \sin{k_{x}} \sigma_{1} + \sin{k_{y}} \sigma_{2} + \left(m + \cos{ k_{x} } + \cos{ k_{y}}\right) \sigma_{3},
\end{align}
QWZ 模型中拓扑相变只可能发生在能隙关闭的四个狄拉克点，因此陈数的变化可以写成chirality和mass sign的组合，也就是
\begin{align}
	\Delta C = -\frac{1}{2} \sum_{i} \chi_{i} \operatorname{sgn}(M_i),
\end{align}
其中手性\(\chi_{i} = \operatorname{sgn}(v_x v_y)\), \(M_i\)是狄拉克点对应的质量项。在QWZ model中，\(m < -2\)时其陈数为\(0\), 因此可以写出总陈数：
\begin{align}
	C_{\mathrm{tot}} = \frac{1}{2} \left[-\operatorname{sgn}\left(m+2\right) + 2 \operatorname{sgn}\left(m\right) - \operatorname{sgn} \left(m-2\right)\right],
\end{align}
所以，我们选择\(m = 1\)， 其对应的陈数为\(C = 1\), 这保证了时间反演对称性破缺。

但是仅仅有时间反演对称性破缺是不够的，对于面内轨道磁化\(M_x, M_y\)来说，我们需要体系以\(z\)轴为旋转轴的对称性，如\(C_{2z}, C_{3z}\)对称性破坏掉；当然镜面对称\(\sigma_{xy}\) 也是需要破坏掉。对于面外轨道磁化\(M_{z}\)来说，我们需要体系以\(x,y\)轴为旋转轴的对称性，如\(C_{2x}\) 破坏掉。

具体来说，单层QWZ model中，其具有\(C_{4z}\)对称性和\(C_{2z}\)对称性。\(C_{4z}\)对称性也就是\(\left(k_{x},k_{y}\right) \rightarrow \left(-k_{y}, k_{x}\right) \), 对于这样一个二分量赝自旋空间中，\(C_{4z}\)对称性的表示可以写成 \(U_{C_{4z}} = e^{-i \frac{\pi}{4} \sigma_{3}} \)， 其满足
\begin{align}
	U_{C_{4z}} h_{\mathrm{QWZ}} \left(\bm{k }\right) U^{\dagger}_{C_{4z}} =  h_{\mathrm{QWZ}} \left( C_{4z} \bm{k }\right),
\end{align}
同理，\(C_{2z}\)对称性的表示为\(U_{C_{2z}} = e^{-i \frac{\pi}{2}\sigma_{3}}\)。也就是说，在我们构建的模型中，这两个对称性必须要破坏掉。

但是这还是不够，如果需要出现面内轨道磁化，也就以为着模型不能是单层的。所以我们需要构建双层的模型，也就是bilayer QWZ model。如果没有interlayer tunneling，哈密顿量可以写成
\begin{align}
	H_{0} \left(\bm{k }\right) = \tau_{0} \otimes h_{\mathrm{QWZ}} \left(\bm{k }\right),
\end{align}
但这仍然无法产生轨道磁化。引入layer Pauli matrices \(\tau_{x,y,z}\)，可以将bilayer QWZ model的z方向位置矩阵写出
\begin{align}
	z = \frac{d}{2} \tau_{z} \otimes \sigma_{0} = \begin{pmatrix}
		\frac{d}{2} & & & \\ & \frac{d}{2} & & \\ & & -\frac{d}{2} & \\ & & & -\frac{d}{2}
	\end{pmatrix},
\end{align}
其中\(d \)是层间距。很明显可以得到\(\left[H,z\right] = 0\), 进而有\(\left[\mathcal{P}, z\right] =0,\, \left[\mathcal{Q}, z\right] = 0\)。 所以\(\mathcal{Z} = \left\{H -\mu ,z\right\} = 2 \left(H - \mu\right) z\)。 而面内轨道磁化有
\begin{align}
	\begin{split}
		\mathcal{F}_{\alpha} &= \operatorname{Tr} \left[\mathcal{P} \left(\partial_{k_{\alpha}} \mathcal{P}\right) \mathcal{Q} \mathcal{Z} \right] = 2 \operatorname{Tr}\left[\mathcal{P} \left(\partial_{k_{\alpha}} \mathcal{P}\right)  \mathcal{Q} \left(H - \mu\right) z \right] \\
		& =  2 \operatorname{Tr}\left[\mathcal{P} \left(\partial_{k_{\alpha}} \mathcal{P}\right)  \left(H - \mu\right) z  \mathcal{Q} \right] \\
		& = 2 \operatorname{Tr}\left[  \mathcal{Q} \mathcal{P} \left(\partial_{k_{\alpha}} \mathcal{P}\right)  \left(H - \mu\right) z  \right] = 0,
	\end{split}
\end{align}
这里最后一步利用求迹的轮换性质，同时\(\mathcal{Q}\mathcal{P} = 0\)， 倒数第二步则利用了\(\left[H - \mu, \mathcal{Q}\right] = 0\)。所以我们必须要引入层间跃迁，那么最简单的层间跃迁也就是\(\tau_{x } \otimes \sigma_{0}\), 合在一起也就是
\begin{align}
	H_{0} \left(\bm{k }\right) = \tau_{0} \otimes h_{\mathrm{QWZ}} \left(\bm{k}\right) + t_{\perp} \tau_{x } \otimes \sigma_{0},
\end{align}
但是这样上下两层是相同的，还存在一个交换上下层的对称性，即\(\ket{t} \rightarrow \ket{b},	\, \ket{b } \rightarrow \ket{t }	\)。记这个对称性为\(L\), \(L \)的表示为\(U_{L } = \tau_{x } \otimes \sigma_{0}\)。这个对称性同样会禁止面内轨道磁化，首先同样有\(\left[H, L\right] = 0\), 因此\(\left[\mathcal{P}, L \right] = \left[\mathcal{Q}, L\right] = 0\), 而对于z方向上坐标算符，上下层交换对称性作用后得到
\begin{align}
	L z L^{-1 } = \frac{d }{2 } \tau_{x } \tau_{z } \tau_{x }^{-1 } \otimes \sigma_{0} = \frac{d }{ 2 } \left(- \tau_{z } \right) \otimes \sigma_{0} = -z,
\end{align}
很容易就可以得到
\begin{align}
	L \mathcal{F}_{\alpha} L^{-1} = - \mathcal{F}_{\alpha}.
\end{align}
也就是\(-\mathcal{F}_{\alpha} = \mathcal{F}_{\alpha} \, \Rightarrow \mathcal{F}_{\alpha} = 0\)。所以我们的目标变成了构造两层不同的QWZ model，在此基础上需要同时破坏掉\(C_{4z}\)和\(C_{2z}\)对称性。那么一种合理的做法是对于上下两层的\(k_{x}\)反向地施加一个动量位移\(\lambda\)， 也就是
\begin{align}
	\boxed{
		\begin{aligned}
			H_\mathrm{BQWZ} \left(\bm{k}, \lambda\right) = \begin{pmatrix}
				h_{\mathrm{QWZ}}\left(k_{x}+\lambda, k_{y}\right) & t_{\perp} \sigma_{0} \\
				t_{\perp} \sigma_{0 } & h_{\mathrm{QWZ}} \left(k_{x} - \lambda , k_{y}\right)
			\end{pmatrix}.
		\end{aligned}
	}
\end{align}
对于正方晶格的倒格矢基矢\(\bm{b}_{1} = \left(2 \pi, 0\right)\) 和 \(\bm{b }_{2} = \left(0, 2 \pi\right)\)，通常选取\(\lambda \ll \left|\bm{b}_{1}\right|\)，这样可以很好地破坏掉两个对称性。

特别地，采用这样的哈密顿量，由于在面内轨道磁化的公式中需要取实部，会导致\(x \)方向上的轨道磁化\(M_{x } = 0\)。 对于没有施加动量位移的单层QWZ 哈密顿量，其满足\(h_{\mathrm{QWZ}}^{* } \left(k_{x},k_{y}\right) = h_{\mathrm{QWZ}} \left(k_{x},-k_{y}\right)\)。加入动量位移后，上下层的哈密顿量同样满足这个关系\(h^{*}\left(k_{x} \pm \lambda , k_{y}\right) = h \left(k_{x} \pm \lambda, -k_{y}\right) \)。由此，双层QWZ模型的哈密顿量和其对应的占据态算符\(\mathcal{P}\)也满足这个关系，即
\begin{align}
	\begin{split}
		H^{*} \left(k_{x},k_{y}\right) &= H \left(k_{x}, -k_{y}\right) \\
		\mathcal{P}^{*} \left(k_{x},k_{y}\right) &= \mathcal{P} \left(k_{x}, -k_{y}\right), 
	\end{split}
\end{align}
对\(\mathcal{P}\)求导有
\begin{align}
	\begin{split}
		\frac{\operatorname{d}}{\operatorname{d} k_{y}} \mathcal{P}^{*} \left(k_{x},k_{y}\right) & = \left[\frac{\operatorname{d}}{\operatorname{d} k_{y}} \mathcal{P} \left(k_{x},k_{y}\right) \right]^{*} = \left[\partial_{k_{y}} \mathcal{P} \left(k_{x},k_{y}\right)\right]^{*} \\
		&  = \frac{\operatorname{d}}{\operatorname{d} k_{y}} \mathcal{P} \left(k_{x}, -k_{y}\right) = - \partial_{k_{y}} \mathcal{P} \left(k_{x}, - k_{y}\right)，
	\end{split}
\end{align}
也就是
\begin{align}
	\partial_{k_{y}} \mathcal{P} \left(k_{x}, - k_{y}\right) = - \left[\partial_{k_{y}} \mathcal{P} \left(k_{x},k_{y}\right)\right]^{*},
\end{align}
取实部的情况下 \(\operatorname{Re} \partial_{k_{y}} \mathcal{P} \left(k_{x}, - k_{y}\right)  = - \operatorname{Re} \partial_{k_{y}} \mathcal{P} \left(k_{x},k_{y}\right)\)。因此\(\mathcal{F}_{y} \left(k_{x}, - k_{y}\right) = - \mathcal{F}_{y} \left(k_{x},  k_{y}\right) \), 而由于布里渊区是关于\(x\)轴对称的，所以积分后\(M_{x } = 0\)。而\(M_{y} \neq 0\)。

\subsection{数值计算结果}
由于我们这里计算的是toy model，所以就直接计算\(\bm{M }\)，也就是热力学极限下单位面积的轨道磁化强度。而不是有限体系中每个晶胞的轨道磁矩\(m_{\mathrm{cell}}\)。

对于我们具体的数值计算，我们所选取的参数如下
\begin{table*}[h!]
	\centering
	\begin{tabular}{c c c c c }
		\toprule
		Parameter & \(m\) & \(t_{\perp}\) & \(\eta\) & \(n_{k}\)  \\
		\midrule
		value & \(1\) & \(0.02\) & \(0.03\)  & \(251\)  \\
		\bottomrule
	\end{tabular}
\end{table*}
其中，我们使用高斯函数来对delta函数进行近似，\(\eta\)是高斯函数的展宽，
\begin{align}
	\delta_{\eta} \left(E - \mu\right) = \frac{1}{\sqrt{\pi} \eta} e^{- \left(E -\mu\right)^2/\eta^{2}},
\end{align}
通常来说我们希望\(\eta \gtrsim \Delta E_{\mathrm{mesh}} = v_{F} \left|\bm{b}_{1}\right|/n_{k}\), 这样可以有效地进行采样。对于QWZ模型，\(v_{F} = 1\), 取\(n_k = 251\), \(\left|\bm{b}_{1}\right|/n_{k} \approx 0.025\), 因此我们取\(\eta = 0.03\)。动量位移取值范围为\(\lambda \in \left[0, 0.2\right]\)。
...
\begin{figure}[h!]
	...
	\centering
	\begin{tabular}{cc}
		\includegraphics[width=0.45\textwidth]{figure/qwz/band/qwz_band_lambda_0.000_m_1.000_tperp_0.200.pdf}
		&
		\includegraphics[width=0.45\textwidth]{figure/qwz/band/qwz_band_lambda_0.200_m_1.000_tperp_0.200.pdf}
	\end{tabular}
	\caption{
		Bilayer QWZ模型能带结构图，其中\(\lambda\)分别为\(0\)和\(0.2\)
	}
	\label{fig1}
\end{figure}
...
\begin{figure}[h!]
	...
	\centering
	\begin{tabular}{cc}
		\includegraphics[width=0.45\textwidth]{figure/qwz/om/qwz_om_fortran_m_1.000_tperp_0.200_mu_0.000.pdf}
		&
		\includegraphics[width=0.45\textwidth]{figure/qwz/om/qwz_om_fortran_m_1.000_tperp_0.200_mu_-1.000.pdf}
	\end{tabular}
	\caption{
		Bilayer QWZ模型轨道磁化\(M_y\)示意图，其中化学势\(\mu\)分别取\(0\)和\(-1\)
	}
	\label{fig2}
\end{figure}

这里通过计算还可以得到\(M_{y}\left(\mu\right) = M_{y} \left(- \mu\right)\), 这是由于particle-hole 对称性得到的结果。对于没有动量位移的单层QWZ模型，很容易可以看出
\begin{align}
	\sigma_{1} h^{*} \left(k_{x}, k_{y}\right) \sigma_{1} = - h\left(-k_{x}, -k_{y}\right),
\end{align}
由此就可以定义单层QWZ模型对应的反幺正算符，定义
\begin{align}
	\mathcal{C}_{0} = \sigma_{1} K,
\end{align}
即可得到
\begin{align}
	\mathcal{C}_{0} h \left(\bm{k}\right) \mathcal{C}_{0}^{-1} = -h \left(- \bm{k}\right).
\end{align}
但是对于双层QWZ模型来说，考虑到存在动量位移和层间跃迁，不可以简单地使用\(\mathcal{C}_{0} = \sigma_{1} K\)。更准确地说，如果继续对每一层还是采用\(\sigma_{1}K\)，得到的是
\begin{align}
	\begin{split}
		\sigma_{1} K h\left(k_{x} + \lambda , k_{y}\right) \left(\sigma_{1} K \right)^{-1} &= - h \left(- k_{x} -\lambda, -k_{y}\right) \\
		\sigma_{1} K h\left(k_{x} - \lambda , k_{y}\right) \left(\sigma_{1} K \right)^{-1} &= - h \left(- k_{x} + \lambda, -k_{y}\right).
	\end{split}
\end{align}
而粒子空穴对称性所要求的则是
\begin{align}
	\mathcal{C} H \left(\bm{k}, \lambda\right) \mathcal{C }^{-1} = - H \left(-\bm{k }, \lambda\right),
\end{align}
具体把每一层写出则是
\begin{align}
	\begin{split}
		- h_{t } \left( -\bm{k }, \lambda\right) &= - h \left( -k_{x} + \lambda, -k_{y}\right) \\
		- h_{b} \left( -\bm{k }, \lambda\right) &= - h \left( -k_{x} - \lambda, -k_{y}\right)
	\end{split}
\end{align}
这意味着我们在对每一层做完\(\sigma_{1} K\)操作后，还需要交换上下层。而首先交换上下层可以想到的就是\(\tau_{x}\)，但是对于层间跃迁来说\(\left(\tau_{x } \otimes \sigma_{1} K\right) \tau_{x } \otimes \sigma_{0} \left(\tau_{x } \otimes \sigma_{1} K \right)^{-1} = \tau_{x } \otimes \sigma_{0}\), 并不会产生一个负号。所以需要使用\(\tau_{y }\), 因此粒子空穴对称性可以写为
\begin{align}
	\mathcal{C} = \tau_{y } \otimes \sigma_{1} K.
\end{align}
进一步，粒子空穴对称性将占据态算符和空态算符进行了互换，具体来说
\begin{align}
	\boxed{
		\begin{aligned}
			\mathcal{C} \mathcal{P}_{\mu} \left(\bm{k}\right) \mathcal{C}^{-1} = \mathcal{Q}_{- \mu} \left(- \bm{k}\right) \\
			\mathcal{C} \mathcal{Q}_{\mu} \left(\bm{k}\right) \mathcal{C}^{-1} = \mathcal{P}_{- \mu} \left(- \bm{k}\right)
		\end{aligned}
	}
\end{align}
而对于\(z\)方向上坐标算符有
\begin{align}
	\mathcal{C} z \mathcal{C}^{-1} = -z,
\end{align}
对于\(\mathcal{Z}\)则有，
\begin{align}
	\mathcal{C} Z_{\mu} \left(\bm{k }\right) \mathcal{C}^{-1} = \left\{-z, - H \left(- \bm{k }  \right) - \mu\right\} = \left\{z, H \left(- \bm{k} \right) + \mu \right\} = \mathcal{Z}_{-\mu} \left(- \bm{k }\right).
\end{align}
由此可以得到
\begin{align}
	\boxed{
		M_{\alpha} \left(\mu, \lambda\right) = M_{\alpha} \left(-\mu, \lambda\right).
	}
\end{align}

而\(\mu = \pm 1\)和\(\mu = 0\)的\(M_{y}\)的符号变号，则应该和填充有关，通过后续的极化率计算，可以看出这一项主要是费米面的变化所导致的。更仔细地说，是对于\(\mu = -1\)时，是价带的一部分从占据态变成空态，而对于\(\mu = 1\), 则是导带的一部分从空态变成占据态。
...
\begin{itemize}
	\item 极化率
\end{itemize}
计划率的计算过程中，我们比较的是通过直接计算的轨道磁化\(\bm{M}\left(\lambda\right)\)与首先选取直接计算轨道磁化的\(\bm{M} \left(0\right) + \int \chi \left(\lambda\right)\operatorname{d} \lambda\), 结果如下
\begin{figure}[h!]
	\captionsetup{list=false}
	\centering
	\begin{tabular}{cc}
		\includegraphics[width=0.5\textwidth]{figure/qwz/susceptibility/m_1.000_tperp_0.200_mu_0.000_eta_0.030_nk_251/chi_y.pdf}
		&
		\includegraphics[width=0.5\textwidth]{figure/qwz/susceptibility/m_1.000_tperp_0.200_mu_0.000_eta_0.030_nk_251/benchmark_My.pdf}
	\end{tabular}
	\caption{
		\(\mu = 0\) 时的\(My\)示意图
	}
...
可以看在没有费米面对轨道磁化强度的贡献下，两者基本上是严格相等。同时\(M_{x}\)的量级大概在\(10^{-18}\)可以认为是零。费米面不为零的贡献如下

\begin{figure}[h!]
	\captionsetup{list=false}
	\centering
	\begin{tabular}{cc}
		\includegraphics[width=0.5\textwidth]{figure/qwz/susceptibility/m_1.000_tperp_0.200_mu_-1.000_eta_0.030_nk_251/chi_y.pdf}
		&
		\includegraphics[width=0.5\textwidth]{figure/qwz/susceptibility/m_1.000_tperp_0.200_mu_-1.000_eta_0.030_nk_251/benchmark_My.pdf}
	\end{tabular}
	\caption{
		\(\mu = -1\) 时的\(My\)示意图
	}
...
可以看到费米面的贡献为主导，同时直接计算\(M_{y}\)和通过极化率重构出来的\(M^{\chi}_{y}\)符合的相当好。通过误差分析可以得到，最大差值约为\(5.6 \times 10^{-5}\), 均方根误差为\(3.27 \times 10^{-5}\), 归一化最大差值为\(6.35 \times 10^{-3}\)，可以认为两者相当的接近。

为什么双层QWZ模型吻合的较好，可能是因为我们引入的扰动\(\lambda\)是一个相对动量平移。
...
\begin{align}
	\begin{split}
		...
		& \operatorname{Re} \left[\braket{u_{n } | \partial_{k_{\beta}} u_{m} } z_{mn } \left(\bm{k }\right)	 \right].
	\end{split}
\end{align}
