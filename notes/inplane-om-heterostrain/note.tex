...
\begin{align}
	...
	\end{split}
\end{align}
这里由于投影算符的导数满足\(\mathcal{P} \mathcal{P}_{\lambda/\alpha} \mathcal{P} = \mathcal{Q} \mathcal{Q}_{\lambda/\alpha} \mathcal{Q} = 0\)，故\(\mathcal{P}_{\lambda/\alpha}\)和\(\mathcal{Q}_{\lambda/\alpha}\)仅含\(\mathcal{P}-\mathcal{Q}\)非对角分量，从而其对易子\(\left[\mathcal{P}_{\lambda}, \mathcal{P}_{\alpha}\right]\)在\(\mathcal{P}/\mathcal{Q}\)空间中是分块对角的，即与\(\mathcal{P}\)对易。因此
\begin{align}
...
\end{align}
所以对于固定秩投影算符的二维区域的求和，变成了对穿过化学势的能带求和（但是这里的积分任然是对费米面（或者是二维区域的边界进行线积分），将上式进行改写
\begin{align}
	\sum_{s} \oint_{\partial \Omega_{s}} \mathrm{d} \ell n_{s, \beta} \mathcal{A}_{s }	= \sum_{m} \oint_{E_{l} = \mu} \mathrm{d} \ell n_{l, \beta} \Delta_{l} \mathcal{A}.
\end{align}
于是第一项的积分可以重新写成
\begin{align}
...
\begin{align}
	H^{\lambda}_{nm} & \equiv \braket{u_{n} | \hat{H}_{\lambda} | u_{m }}, \\
...
进一步将\(\mathcal{A}\)写成
\begin{align}
	\mathcal{A} = \mathrm{Re} \sum_{\substack{n \in \mathrm{occ} \\ m \in \mathrm{emp}}} \frac{E_{n} + E_{m } - 2 \mu}{E_{n } - E_{m }} H^{\lambda}_{nm} z_{mn}.
\end{align}
...
\begin{align}
	\begin{split}
		\mathcal{A}_{-}  = \mathrm{Re} \left[\sum_{n \in O} \sum_{m \in E} + \sum_{n =l} \sum_{m \in E}\right] = \mathcal{A}_{0} + \mathrm{Re} \sum_{m \in E} \frac{E_{l} + E_{m } - 2 \mu}{E_{l} - E_{m }} H^{\lambda}_{lm} z_{ml} ,
	\end{split}
\end{align}
其中
\begin{align}
	\mathcal{A}_{0} = \mathrm{Re} \sum_{n \in O} \sum_{m \in E} \frac{E_{n} + E_{m } - 2 \mu}{E_{n } - E_{m }} H^{\lambda}_{nm} z_{mn}.
\end{align}
那么在\(\Omega_{+}\)中，则有
\begin{align}
	\begin{split}
		\mathcal{A}_{+} =  \mathrm{Re} \left[\sum_{n \in O} \sum_{m \in E} + \sum_{n \in O} \sum_{m = l}\right] = \mathcal{A}_{0} + \mathrm{Re} \sum_{n \in O } \frac{E_{n} + E_{l} - 2 \mu}{E_{n } - E_{l}} H^{\lambda}_{nl} z_{ln}.
	\end{split}
\end{align}
...
所以
\begin{align}
	\mathcal{A}_{+} = \mathcal{A}_{0} + \mathrm{Re} \sum_{n \in O } H^{\lambda}_{nl} z_{ln}.
\end{align}
两者的差为
\begin{align}
	\mathcal{A}_{-} - \mathcal{A}_{+} = - \mathrm{Re} \sum_{m \in E } H^{\lambda}_{lm } z_{m l} - \mathrm{Re} \sum_{n \in O } H^{\lambda}_{nl} z_{ln},
\end{align}
由于\(H^{\lambda}_{lm }\)和\( z_{m l} \)都是厄米的，所以有 \(H^{\lambda}_{ml} = \left(H^{\lambda}_{lm}\right)^{*}\)，同时\(z_{lm} = z^{*}_{lm }\), 所以
\begin{align}
	\mathrm{Re} H^{\lambda}_{lm} z_{ml} = \mathrm{Re} \left( H^{\lambda}_{ml} z_{lm} \right)^{*} = \mathrm{Re} H^{\lambda}_{ml} z_{lm},
\end{align}
可以将上式改写成
\begin{align}
	\mathcal{A}_{-} - \mathcal{A}_{+} = - \mathrm{Re} \sum_{m \in E}  H^{\lambda}_{lm} z_{ml} - \sum_{n \in O } H^{\lambda}_{ln} z_{nl}  = - \mathrm{Re} \sum_{r \neq l} H^{\lambda}_{lr} z_{rl}
\end{align}
这里我们使用了一个虚拟变量\(r\)，为了符合我们的使用习惯，我们做指标替换\(r \rightarrow n\)和\(l \rightarrow m\)。因此，积分为
\begin{align}
	I_{\alpha, 1} = - \int_{\mathrm{mBZ}} \frac{\mathrm{d}^{2} \bm{k}}{\left(2 \pi\right)^2} \sum_{m} \delta \left(E_{m} - \mu\right) \left(\partial_{k_{\alpha}} E_{m }\right) \sum_{n \neq m } \mathrm{Re} \left[H^{\lambda}_{mn} z_{nm}\right]，
\end{align}
...
\begin{align}
	\begin{split}
		...
		\Rightarrow \quad \nabla_{\bm{k}} E_{l } \cdot \frac{\partial \bm{k}_{\mathrm{FS}}}{\partial \lambda} & = - \partial_{\lambda} E_{l }	= - H^{\lambda}_{ll},
	\end{split}
\end{align}
又有\(\nabla_{\bm{k }} E_{l } = \left|\nabla_{\bm{k }} E_{l } \right| \hat{\bm{n }}_{l, \lambda}\), 由此
\begin{align}
	\begin{split}
		\left|\nabla_{\bm{k }} E_{l } \right| \hat{\bm{n }}_{l, \lambda} \cdot \frac{\partial \bm{k}_{\mathrm{FS}}}{\partial \lambda} &= - H^{\lambda}_{ll} \\
		\Rightarrow \quad  v_{l,\lambda} &=  - \frac{H^{\lambda}_{ll}}{\left|\nabla_{\bm{k}} E_{l}\right|}.
	\end{split}
\end{align}
定义
\begin{align}
...
可以写出费米面对极化率的第二项贡献为
\begin{align}
	\chi^{\mathrm{FS}}_{\alpha,2} =  \frac{e }{\hbar c} \epsilon_{\alpha \beta} \sum_{s } \oint_{\partial \Omega_{s }} \frac{\mathrm{d} \ell}{ \left(2 \pi\right)^2} \frac{H^{\lambda}_{ll}}{\left|\nabla_{\bm{k}} E_{l }\right|} \Delta_{l } \mathrm{Re} \mathcal{F}_{\beta},
\end{align}
将线积分改写成面积分
\begin{align}
	\chi^{\mathrm{FS}}_{\alpha,2} =  \frac{e }{\hbar c} \epsilon_{\alpha \beta} \sum_{l} \int_{\mathrm{mBZ}} \frac{\mathrm{d}^{2} \bm{k}}{\left(2 \pi\right)^2} \delta \left(E_{l} - \mu\right) H^{\lambda}_ll \Delta_{l} \mathrm{Re} \mathcal{F}_{\beta}.
\end{align}
现在来处理\(\Delta_{l} \mathrm{Re} \mathcal{F}_{\beta}\), 由于
\begin{align}
	\mathrm{Re} \mathcal{F}_{\beta} = -\sum_{\substack{n \in \mathrm{occ} \\ m \in \mathrm{emp}}} \left(E_{n } + E_{m } - 2\mu\right) \mathrm{Re} \braket{u_{n } | \partial_{k_{\beta}} u_{m }} \braket{u_{m } | \hat{z} | u_{n }}.
\end{align}
对\(\Omega_{-}\)这一侧区域有
\begin{align}
	\mathrm{Re} \mathcal{F}^{-}_{\beta} = \mathcal{F}_{\beta}^{0} - \sum_{m \in E} \left(E_{m} + E_{l} - 2\mu\right) \mathrm{Re} \braket{u_{l} | \partial_{k_{\beta}} u_{m }} \braket{u_{m } | \hat{z} | u_{l}}.
\end{align}
对\(\Omega_{+}\)有
\begin{align}
	\mathrm{Re} \mathcal{F}^{+}_{\beta} = \mathcal{F}_{\beta}^{0} - \sum_{n \in O} \left(E_{l} + E_{n} - 2\mu\right) \mathrm{Re} \braket{u_{n} | \partial_{k_{\beta}} u_{l}} \braket{u_{l } | \hat{z} | u_{n }}.
\end{align}
其中
\begin{align}
	\mathcal{F}_{\beta}^{0} = - \mathrm{Re} \sum_{\substack{n \in O \\ m \in E}} \left(E_{n } + E_{m } - 2 \mu\right) \mathrm{Re} \braket{u_{n } | \partial_{k_{\beta}} u_{m }} \braket{u_{m } | \hat{z} | u_{n }}.
\end{align}
所以
\begin{align}
	\begin{split}
			\Delta_{l} \mathrm{Re} \mathcal{F}_{\beta} &= \mathrm{Re} \mathcal{F}^{-}_{\beta} - \mathrm{Re} \mathcal{F}^{+}_{\beta}	 \\
			& = - \sum_{m \in E} \left(E_{m} + E_{l} - 2\mu\right) \mathrm{Re} \braket{u_{l} | \partial_{k_{\beta}} u_{m }} \braket{u_{m } | \hat{z} | u_{l}} \\
			& \quad + \sum_{n \in O} \left(E_{l} + E_{n} - 2\mu\right) \mathrm{Re} \braket{u_{n} | \partial_{k_{\beta}} u_{l}} \braket{u_{l } | \hat{z} | u_{n }} .
	\end{split}
\end{align}
又
\begin{align}
	\mathrm{Re} \braket{u_{l} | \partial_{k_{\beta}} u_{m }} \braket{u_{m } | \hat{z} | u_{l}} = - \mathrm{Re} \bra{u_m | \partial_{k_{\beta}} u_{l}} \braket{u_{l} | \hat{z} | u_{m}}.
\end{align}
所以
\begin{align}
	\begin{split}
		\Delta_{l} \mathrm{Re} \mathcal{F}_{\beta} & = \sum_{n \in O} \left(E_n - \mu\right) \mathrm{Re} \braket{u_n | \partial_{k_{\beta}} u_{l }} \braket{u_{l} | \hat{z} | u_{n}} + \sum_{m \in E}  \left(E_m - \mu\right) \mathrm{Re} \braket{u_m | \partial_{k_{\beta}} u_{l }} \braket{u_{l} | \hat{z} | u_{m}}, \\
		& = \sum_{r \neq l} \left(E_{r } - \mu\right) \mathrm{Re} \braket{u_{r } | \partial_{k_{\beta}} u_{l }} \braket{u_{l} | \hat{z} | u_{r}}, \\
		& = - \sum_{r \neq l} \left(E_{r } - \mu\right) \mathrm{Re} \braket{u_{l} | \partial_{k_{\beta}} u_{r}} \braket{u_{r} | \hat{z} | u_{l}}
	\end{split}
\end{align}
写出对极化率的贡献为
\begin{align}
	\chi^{\mathrm{FS}}_{\alpha,2} = - \frac{e }{\hbar c } \epsilon_{\alpha \beta} \int_{\mathrm{mBZ}} \frac{\mathrm{d}^{2} \bm{k}}{\left(2 \pi\right)^2} \sum_{l} \sum_{r \neq l} \left(E_r - \mu\right) \delta \left(E_{l} - \mu\right) H^{\lambda}_{ll} \mathrm{Re} \left[\braket{u_{l} | \partial_{k_{\beta}} u_r} z_{rl}  \right],
\end{align}
将
\begin{align}
...
\begin{align}
	\boxed{
		\chi^{\mathrm{FS}}_{\alpha,2} = - \frac{e }{\hbar c } \epsilon_{\alpha \beta} \int_{\mathrm{mBZ}} \frac{\mathrm{d}^{2} \bm{k}}{\left(2 \pi\right)^2} \sum_{m} \sum_{n \neq m} \delta \left(E_{l} - \mu\right) \braket{u_m | \hat{H}_{\lambda} | u_{m} } \mathrm{Re} \left[\braket{u_{m} | \hat{H}_{\beta} | u_n} \braket{u_{n} | \hat{z} | u_{m}} \right],
	}
\end{align}
由此，极化率中来自费米面的贡献为
\begin{align}
	\begin{split}
		\boxed{
			\begin{aligned}
				& \chi^{\mathrm{FS}}_{\alpha} = \frac{e }{\hbar c } \epsilon_{\alpha \beta} \int_{\mathrm{mBZ}} \frac{\mathrm{d}^{2} \bm{k}}{\left(2 \pi\right)^2} \sum_{m} \sum_{n \neq m} \delta \left(E_{l} - \mu\right) \\
