...
\begin{align}
	\hat{H}^{\mathrm{intra}}_{j,\xi} \simeq \frac{1}{A_\mathrm{mlg}} \sum_{S,S^{\prime}} \sum_{\bm{G}_{j}} e^{-i \bm{G}_{j} \cdot \left( \bm{\tau}_{j,S } - \bm{\tau}^{\prime}_{j, S^{\prime}}\right)} \int \mathrm{d} \bm{r} \mathrm{d} \bm{d} e^{i \left(\bm{G}_{j} - \bm{K }_{j,\xi}\right) \cdot \bm{d }} \, t_{jj} \left(\bm{d }\right) \hat{\psi}^{\dagger}_{j, S, \xi} \left(\bm{r}\right) \hat{\psi}_{j,S^{\prime}, \xi} \left(\bm{r} - \bm{d }\right).
\end{align}
将连续场展开到一阶，
\begin{align}
...
\begin{align}
	1 + e^{i 2 \pi \xi/3} + e^{ - i 2 \pi \xi/3} = 0,
\end{align}
...
对于一阶项，定义速度矩阵元
\begin{align}
	v_{j,\xi, S S^{\prime}; \mu} = - \frac{i }{\hbar} M_{j,\xi, S S^{\prime}; \mu}^{\left(1\right)}，
\end{align}
一阶矩可以写成
\begin{align}
	M_{j,\xi, S S^{\prime}; \mu}^{\left(1\right)} = - - \hbar v_{j,\xi, S S^{\prime}; \mu} ,
\end{align}
所以
\begin{align}
	- M_{j,\xi, S S^{\prime}; \mu}^{\left(1\right)} \partial_{\mu} = -i \hbar v_{j,\xi, S S^{\prime}; \mu} \partial_{\mu} = \hbar v_{j,\xi, S S^{\prime}; \mu} \left(-i \partial_{\mu}	\right),
\end{align}
一阶哈密顿量可以写成
\begin{align}
	\hat{H }^{\mathrm{intra}; \left(1\right)} _{j,\xi} = \hbar \int \mathrm{d}^{2} \bm{r}  \hat{\psi}^{\dagger}_{j,\xi} \left(\bm{r}\right) \left[v_{j,\xi;x } \left(-i \partial_{x }\right) + v_{j,\xi;y} \left(-i \partial_{y}\right)\right] \hat{\psi_{j,\xi}} \left(\bm{r}\right).
\end{align}
其中\(v_{j,\xi;x/y}\)是\(2 \times 2\)厄米矩阵。将其写成
\begin{align}
	h^{\left(1\right)}_{j,\xi} = \hbar \sum_{\mu=x,y} v_{j,\xi;\mu} \left(-i \partial_{\mu}\right). 
\end{align}
速度矩阵的具体形式为
\begin{align}
	\boxed{
		v_{j,\xi, S S^{\prime};\mu} = - \frac{i}{\hbar} \frac{1}{A_{\mathrm{mlg}}} \sum_{\bm{G}_{j}} e^{-i \bm{G}_{j} \cdot \left(\bm{\tau}_{j,S} - \bm{\tau}^{\prime}_{j,S^{\prime}}\right)} \int \mathrm{d}^{2} \bm{d } d_{\mu} t_{jj} \left(\bm{d }\right)  e^{i \left(\bm{G}_{j} - \bm{K }_{j,\xi} \right) \cdot \bm{d }}.
	}
\end{align}
记跃迁傅里叶变换的哑变量为\(\mathcal{Q }\), 定义
\begin{align}
	\tilde{t } \left(\mathcal{Q}\right) = \frac{1}{A_{\mathrm{mlg}}} \int \mathrm{d}^{2} \bm{d } \, t \left(\bm{d }\right) e^{-i \mathcal{Q }\cdot\bm{d }},
\end{align}
求导有
\begin{align}
	\frac{\partial \tilde{t } \left(\mathcal{Q}\right) }{\partial \mathcal{Q }_{\mu}} = - \frac{i }{A_{\mathrm{mlg}}} \int \mathrm{d}^{2} \bm{d } \, t \left(\bm{d }\right) d_{\mu} e^{-i \mathcal{Q }\cdot\bm{d }}.
\end{align}
所以速度矩阵可以表示成
\begin{align}
	\hbar v_{j,\xi,SS^{\prime};\mu} =  \sum_{\bm{G}_{j}}  e^{-i \bm{G}_{j} \cdot \left(\bm{\tau}_{j,S} - \bm{\tau}^{\prime}_{j,S^{\prime}}\right)} \partial_{\mathcal{Q }_{\mu}} \tilde{ t } \left(\bm{Q }\right) \bigg|_{\mathcal{Q } = \bm{K}_{j,\xi} - \bm{G}_{j}}.
\end{align}
即\(v_{\mu}\)是跃迁傅里叶振幅在谷附近的导数。（当然如果觉得这里的减号太难看，也可以将求和重新编号\(\bm{G}_j \mapsto - \bm{G}_{j}\), 就可以得到\(\bm{K} + \bm{G }\)了。）

回到速度举证元的具体形式中，定义
\begin{align}
	\boxed{
		\delta \bm{\tau}_{SS^{\prime}} = \bm{\tau}_{j,S} - \bm{\tau}^{\prime}_{j, S^{\prime}}.
	}
\end{align}
整理e指数得到
\begin{align}
	e^{-i \bm{G}_{j} \cdot \delta \bm{\tau}_{SS^{\prime}}} e^{	i \bm{G}_{j} \cdot\bm{d }} = e^{i \bm{G}_{j} \cdot \left(\bm{d } - \delta \bm{\tau}_{SS^\prime}\right)}.
\end{align}
利用Poisson求和公式，
\begin{align}
	\frac{1}{A_{\mathrm{mlg}}} \sum_{\bm{G}_{j}} e^{i \bm{G}_{j} \cdot \left(\bm{d} - \delta \bm{\tau}_{SS^{\prime}}\right)} = \sum_{\bm{R}} \delta^{\left(2\right)} \left(\bm{d } - \delta \bm{\tau}_{SS^{\prime}} - \bm{R} \right) .
\end{align}
速度矩阵元为
\begin{align}
	\boxed{
		\hbar v_{j,\xi, SS^{\prime};\mu} = -i \sum_{\bm{R}} e^{-i \bm{K }_{j,\xi} \cdot \left( \delta \bm{\tau}_{SS^{\prime}} + \bm{R}\right)} \left( \delta \bm{\tau}_{SS^{\prime}} + \bm{R}\right)_{\mu} t_{jj} \left( \delta \bm{\tau}_{SS^{\prime}} + \bm{R}\right).
	}
\end{align}
现在定义复速度矢量
\begin{align}
	\bm{v }_{SS^{\prime}} = v_{SS^{\prime};x} \hat{x} + v_{SS^{\prime};y} \hat{y}.
\end{align}
再写一遍其定义
\begin{align}
	\hbar \bm{v }_{SS^{\prime}} = -i \sum_{\bm{R}} e^{-i \bm{K }_{j,\xi} \cdot \left( \delta \bm{\tau}_{SS^{\prime}} + \bm{R}\right)} \left( \delta \bm{\tau}_{SS^{\prime}} + \bm{R}\right) t_{jj} \left( \delta \bm{\tau}_{SS^{\prime}} + \bm{R}\right).
\end{align}
由此一阶哈密顿量可以写成
\begin{align}
	h^{\left(1\right)} = \hbar \left[v_x \left(-i \partial_{x }\right) + v_y \left(-i \partial_{y}\right)\right].
\end{align}

现在我们通过对称性分析，得到\(\bm{v }_{SS^{\prime}}\)的具体形式。对于未旋转的单层石墨烯，其具有\(C_{3z}\)对称性。根据我们所选取的convention，首先有
\begin{align}
	\delta \bm{\tau}_{AB} = \bm{\tau}_{A} - \bm{\tau}_{B} = - \frac{1}{3} \left(\bm{a }_{1} + \bm{a }_{2}\right).
\end{align}
同时，实空间中，我们选定旋转轴在\(A\)原子上方，动量空间中的旋转轴在\(\bm{\Gamma }\)点上方。可以得到，实空间中 \(C_{3z} \bm{\tau}_{A } = \bm{\tau}_{A },\quad C_{3z} \bm{\tau}_{B} = \bm{\tau}_{B} - \bm{a}_{1}\), 由此
\begin{align}
	C_{3z} \delta \bm{\tau}_{AB} = \delta \bm{\tau}_{AB}  + \bm{a }_{1}.
\end{align}
动量空间中，对于我们所选定的谷，\(\bm{K} = \frac{1}{3} \left(\bm{b}_{1} - \bm{b}_{2}\right)\), 旋转后有
\begin{align}
	C_{3z} \bm{K} = \bm{K} + \bm{b}_{2}.
\end{align}
当\(S = S^{\prime}\)时，\(\delta \bm{\tau}_{AA} = \delta \bm{\tau}_{BB} = 0\)速度矢量为
\begin{align}
	\hbar \bm{v }_{SS} = - i \sum_{\bm{R}} e^{-i \bm{K} \cdot \bm{R}} \bm{R} t_{SS 	 \left(\bm{R}\right)} ,\quad  S = A, B.
\end{align}
\(C_{3z}\)作用后
\begin{align}
	\begin{split}
		\hbar C_{3z}  \bm{v }_{SS} \left(\bm{K}\right) & = -i \sum_{\bm{R}} e^{-i \bm{K }\cdot\bm{R}} C_{3z} \bm{R} t_{SS } \left(\bm{R}\right)  \\
		& = -i \sum_{\bm{R}^{\prime}} e^{-i \bm{K } \cdot C_{3z}^{-1} \bm{R}^{\prime}} \bm{R}^{\prime} t_{SS} \left(\bm{R}^{\prime}\right) = -i \sum_{\bm{R}^{\prime}} e^{-i C_{3z} \bm{K } \cdot \bm{R}^{\prime}} \bm{R}^{\prime} t_{SS} \left(\bm{R}^{\prime}\right) \\
		& = \hbar  \bm{v }_{SS} \left(C_{3z} \bm{K}\right).
	\end{split}
\end{align}
所以有
\begin{align}
	C_{3z} \bm{v }_{SS} \left(\bm{K}\right) = \bm{v }_{SS} \left(C_{3z} \bm{K}\right).
\end{align}
而
\begin{align}
	\bm{v }_{SS} \left(C_{3z} \bm{K}\right) = \bm{v}_{SS} \left(\bm{K} + \bm{b}_2\right) = - \frac{i}{\hbar} \sum_{\bm{R}} e^{-i \left(\bm{K} + \bm{b}_{2}\right)\cdot \bm{R}} \bm{R} t_{SS} \left(\bm{R}\right) = \bm{v }_{SS} \left(\bm{K}\right),
\end{align}
这里\(e^{-i \bm{b}_{2} \cdot\bm{R}} = 0\). 所以
\begin{align}
	C_{3z} \bm{v }_{SS} \left(\bm{K}\right) =\bm{v }_{SS} \left(\bm{K}\right) .
\end{align}
又\(C_{3z}\)的本征值为\(e^{\pm i 2 \pi/3}\)，而不是\(1\)。如果本征值为\(1\),则必须为零矢量，所以
\begin{align}
	\begin{split}
		v_{SS;x } = v_{SS;y} &= 0 \\
		\bm{v}_{AA} = \bm{v}_{BB} &= 0,
	\end{split}
\end{align}
如果\(S \neq S^{\prime}\), 记\(\mathcal{D }_{AB} = \left\{\bm{R}+ \delta \bm{\tau}_{AB}\right\}, \quad \bm{d } = \bm{R}+ \delta \bm{\tau}_{AB}\)。 这里\(\mathcal{D }_{AB}\)在\(C_{3z}\)下保持不变，同时\(t \left(C_{3z} \bm{d }\right) = t \left(\bm{d }\right)\)。同样可以得到
\begin{align}
	\bm{v }_{AB } \left(\bm{K }\right) = \bm{v }_{AB } \left(C_{3z} \bm{K }\right),
\end{align}
但不同的是
\begin{align}
	\begin{split}
		\bm{v }_{AB } \left(C_{3z} \bm{K }\right) &= - \frac{i }{\hbar} \sum_{\bm{d } \in \mathcal{D }_{AB}} e^{-i \left(\bm{K } + \bm{b }_2\right)\cdot \bm{d }} \bm{d } t \left(\bm{d}\right)  = e^{-i \bm{b }_{2} \cdot \delta \bm{\tau}_{AB} } \bm{v }_{AB} \left(\bm{K}\right) \\
		& = e^{i \frac{2 \pi}{3}} \bm{v }_{AB } \left(\bm{K }\right),
	\end{split}
\end{align}
所以
\begin{align}
	C_{3z} \bm{v }_{AB } \left(\bm{K }\right) = e^{i \frac{2 \pi}{3}} \bm{v }_{AB } \left(\bm{K }\right).
\end{align}
而\(C_{3z}\)的本征态为
\begin{align}
	\begin{split}
		C_{3z} \left(\hat{x} + i \hat{y}\right) &= e^{ - i \frac{2 \pi}{3}} \left(\hat{x} + i \hat{y}\right) ,\\
		C_{3z} \left(\hat{x} - i \hat{y}\right) &= e^{ i \frac{2 \pi}{3}} \left(\hat{x} - i \hat{y}\right).
	\end{split}
\end{align}
所以\(\bm{v }_{AB}\)正比于\(\hat{x } - i \hat{y}\), 记为
\begin{align}
	\bm{v }_{AB} = c \left(\hat{x } - i \hat{y}\right).
\end{align}

接下来利用镜面对称性\(m_y\), 也就是\(m_{y} : \left(x,y\right) \mapsto \left(-x, y\right)\)。其在坐标空间中对应的表示为
\begin{align}
	P_y = \begin{pmatrix}
		-1 & 0 \\ 0 & 1
	\end{pmatrix},
\end{align}
同时\(P_y \bm{K} = - \bm{K }\)。 与之前类似的可以得到
\begin{align}
	P_y \bm{v }_{AB} \left(\bm{K}\right) = \bm{v }_{AB} \left( - \bm{K}\right)
\end{align}
所以
\begin{align}
	\bm{v }_{AB} \left( - \bm{K}\right) = - \frac{i}{\hbar} \sum_{\bm{d }} e^{-i \bm{K }\cdot \bm{d}} \bm{d } t \left(\bm{d }\right) = - \bm{v }_{AB}^{*} \left(\bm{K}\right).
\end{align}
请注意这里的共轭是对整体共轭，所以最前面的\(i \rightarrow -i\)。所以，\(\bm{v }_{AB}\)的形式进一步被限制成
\begin{align}
	P_y \bm{v }_{AB} = c \left(- \hat{x } -i \hat{y}\right) = - c^{*} \left(\hat{x } + i \hat{y}\right),
\end{align}
所以
\begin{align}
	c = c^{*},
\end{align}
也就是\(c \in \mathbb{R }\)必为一实数, 记\(c = v_{F}\)，有
\begin{align}
	\begin{split}
		\bm{v }_{AB} & =v_{F} \left(\hat{x} - i \hat{y}\right) \\
		\bm{v }_{BA} & = v_{F} \left(\hat{x} + i \hat{y}\right).
	\end{split}
\end{align}
整理就可以得到
\begin{align}
	\boxed{
		h^{\left(1\right)}  = \hbar v_{F} \left[\sigma_{1} \left(-i \partial_{x }\right) + \sigma_{2} \left(-i \partial_{y}\right)\right].
	}
\end{align}
其中
\begin{align}
	\boxed{
		v_{F} = -  \frac{i }{\hbar} \sum_{\bm{R }} e^{-i \bm{K } \cdot \left(\bm{R } + \delta \bm{\tau}_{AB}\right)}  \left(\bm{R } + \delta \bm{\tau}_{AB}\right)_{x } t  \left(\bm{R } + \delta \bm{\tau}_{AB}\right).
	}
\end{align} 
\vspace{2em}

等价地，若直接从低能连续理论出发，\(C_{3z}\)允许的一般一阶项可以写为
\begin{align}
	h^{\left(1\right)}_{\xi} \left(\bm{q }\right) = v_1 \left(\xi q_x \sigma_{1} + q_y \sigma_{2}\right) + v_2 \left(\xi q_x \sigma_{2} - q_y \sigma_{1}\right)
\end{align}
这一表达式与上述结果中通过\(c = v_1 -i v_2\)对应。\(m_y\)对称性要求\(c \)为实数，等价于\(v_{2} = 0\). \footnote{真写不动了，下个假期再写，2026年7月31日}
