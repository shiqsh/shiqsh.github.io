...
这里我们再次重新声明三种不同的面积，首先是对应上面公式中的原胞面积\(A_{\mathrm{uc}}\), 在下文中等价于单层石墨烯原胞面积\(A_{\mathrm{mlg}}\), 同时定义莫尔超晶格原胞面积\(A_{\mathrm{M}}\)， 而\(\mathcal{A}\)为整个有限体系的总面积。对于一个同时与原子晶格与莫尔晶格相容的有限体系，有
\begin{align}
...
\begin{align}
	\hat{c}_{l,\alpha,\bm{R}},
\end{align}
其中\(l \in \left\{t, b\right\}\)是层指标，\(\alpha \in \left\{A, B\right\}\)是子晶格指标；同时，上下每一层石墨烯原胞位置表示为\(\bm{R} = n_1 \bm{a}_{1} + n_{2} \bm{a}_{2}\)。（这里我们忽略了自旋）我们记上层的转角为\(\theta_{t} = + \frac{\theta}{2}\), 下层的转角为\(\theta_{b} = -\frac{\theta}{2}\), 定义旋转矩阵 \(\mathcal{R} = R\left(\theta_{l}\right)\)。所以，第\(l\)层的实际格矢为\(\bm{R}_{l} = \mathcal{R}_{l} \bm{R}\)，实际原子位置为
\begin{align}
	\bm{r}_{l,\alpha,\bm{R}} = \mathcal{R}_{l} \left(\bm{R} + \bm{\tau}_{\alpha}\right) + z_{l} \hat{z}.
\end{align}
对于刚性旋转，\(\det \mathcal{R}_{l} = 1\), 所以旋转前后单层石墨烯的面积均为\(A_{\mathrm{mlg}}\)。

首先，对每一层的格点算符做傅立叶变换，
\begin{align}
	\hat{c}_{l,\alpha, \bm{R}} = \frac{1}{\sqrt{N_0}} \sum_{\bm{p} \in \mathrm{BZ}_{l}} e^{i \bm{p} \cdot \bm{R}_{l}} \hat{c}_{l, \alpha, \bm{p}}.
\end{align}
其中\(\bm{p}\)类似我们前文中所讨论的\(\bm{k}\)。低能态集中在两个谷附近，所以把动量写成
\begin{align}
	\bm{p} = \bm{K}_{l, \xi} + \bm{q}, \quad \xi  = \pm, \quad \left|\bm{q}\right| < \Lambda.
\end{align}
只保留两个谷附近的低能态
\begin{align}
	\hat{c}_{l, \alpha, \bm{R}} \simeq \frac{1}{\sqrt{N_0}} \sum_{xi = \pm} \sum_{\left|\bm{q}\right| < \Lambda} e^{i \left(\bm{K}_{l, \xi} + \bm{q}\right) \cdot \bm{R}_{l}} \hat{c}_{l, \alpha, \bm{K}_{l, \xi} + \bm{q}},
\end{align}
提出每个谷分别对应的载波相位
\begin{align}
	\hat{c}_{l, \alpha, \bm{R}} \simeq \frac{1}{\sqrt{N_0}} \sum_{xi = \pm}  e^{i \bm{K}_{l, \xi} \cdot \bm{R}_{l}} \sum_{\left|\bm{q}\right| < \Lambda} e^{i \bm{q} \cdot \bm{R}_{l} } \hat{c}_{l, \alpha, \bm{K}_{l, \xi} + \bm{q}}.
\end{align}
由此，我们可以定义连续包络场、
\begin{align}
	\hat{\psi}_{l, \alpha, \xi} \left(\bm{r}\right) = \frac{1}{\sqrt{\mathcal{A}}} \sum_{\left|\bm{q}\right| < \Lambda} e^{i \bm{q} \cdot \bm{r}} \hat{c}_{l, \alpha,  \bm{K}_{l, \xi} + \bm{q}}
\end{align}	
利用
\begin{align}
	\mathcal{A} = N_0 A_{\mathrm{mlg}}.
\end{align}
得到
\begin{align}
	\boxed{
			\hat{c}_{l, \alpha, \bm{R}} \simeq \sqrt{A_{\mathrm{mlg}}} \sum_{\xi = \pm} e^{i \bm{K}^{l , \xi} \cdot \bm{R}_{l}} \hat{\psi}_{l, \alpha, \xi} \left(\bm{R}_{l}\right). 
	}
\end{align}
这里每一个完整的连续场\(\hat{\psi}_{l, \alpha, \xi} \left(\bm{r}\right)\)有八个分量。
